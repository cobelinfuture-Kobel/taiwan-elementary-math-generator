...
const SPEC_BY_ID = new Map(CANONICAL_SPECS.map((spec) => [spec.patternSpecId, spec]));
...
export function allocateG5AU02HiddenWorksheet(input = {}) {
  ...
  return deepFreeze({ ok: true, errors: [], plan: normalized.plan, allocation });
}
...
export function formatG5AU02Answer(answerModelId, answer) {
  ...
  switch (answerModelId) {
    ...
    case "factorPairListAnswer":
      return answer.pairs.map(([left, right]) => `${left}×${right}`).join("、");
    case "orderedFactorRelationAnswer":
      return `因數：${joinValues(answer.factorList)}；配對：${answer.symmetricPairs.map(([left, right]) => `${left}×${right}`).join("、")}`;
    case "missingValueMapAnswer":
      return Object.entries(answer.valuesByPosition)
        .sort(([left], [right]) => Number(left) - Number(right))
        .map(([position, value]) => `第 ${Number(position) + 1} 格=${value}`)
        .join("、");
...
export function buildG5AU02HiddenWorksheetDocument(input = {}) {
  ...
  return deepFreeze({ ok: true, errors: [], plan: allocated.plan, worksheetDocument });
}
...
export function validateG5AU02HiddenWorksheetDocument(document) {
  ...
  if (!document || typeof document !== "object") {
    return deepFreeze({ ok: false, errors: ["G5AU02_WORKSHEET_DOCUMENT_REQUIRED"] });
  }
  ...
  if (document.answerKeyEnabled
    && JSON.stringify(flattenedRecords(document.answerKeyPages ?? [])) !== JSON.stringify(answers)) {
    errors.push("G5AU02_WORKSHEET_ANSWER_PAGINATION_MISMATCH");
  }

  const allocatedTotal = Object.values(document.allocation?.patternCounts ?? {})
...
export function auditG5AU02HiddenWorksheetIntegration() {
  const errors = [];
  const answerModels = [...new Set(CANONICAL_SPECS.map((spec) => spec.answerModel.shape))];
  ...
  if (answerModels.length !== 16) errors.push("G5AU02_WORKSHEET_ANSWER_MODEL_COUNT_MISMATCH");
  for (const answerModelId of answerModels) {
    if (!SUPPORTED_ANSWER_MODEL_SET.has(answerModelId)) {
      errors.push(`G5AU02_WORKSHEET_ANSWER_MODEL_UNSUPPORTED:${answerModelId}`);
    }
  }
  ...
  if (!full.ok) errors.push(...full.errors);
    ...
    const validation = validateG5AU02HiddenWorksheetDocument(full.worksheetDocument);
    if (!validation.ok) errors.push(...validation.errors);
  }
  const suppressed = buildG5AU02HiddenWorksheetDocument({
    patternSpecIds: CANONICAL_IDS.slice(0, 3),
    questionCount: 7,
    baseSeed: 191,
    includeAnswerKey: false,
  });
  if (!suppressed.ok) errors.push(...suppressed.errors);
  else if (suppressed.worksheetDocument.answerKeyRecords.length !== 0
    || suppressed.worksheetDocument.answerKeyPages.length !== 0) {
    errors.push("G5AU02_WORKSHEET_ANSWER_SUPPRESSION_FAILED");
  }
  ...
  return deepFreeze({
    ok: errors.length === 0,
    errors: [...new Set(errors)],
    patternSpecCount: CANONICAL_SPECS.length,
    classCCount,
    classDCount,
    answerModelCount: answerModels.length,
    supportedAnswerModelCount: SUPPORTED_ANSWER_MODEL_IDS.length,
  });
}

export function getG5AU02HiddenWorksheetPatternIds() {
  return [...CANONICAL_IDS];
}

export function getG5AU02HiddenWorksheetAnswerModelIds() {
