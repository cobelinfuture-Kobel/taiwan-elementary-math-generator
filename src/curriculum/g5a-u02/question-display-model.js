...
const SOURCE_PASSWORD_CONDITIONS = Object.freeze([
  Object.freeze({
    conditionId: "third_digit_common_factor",
    text: "第三個數字和第一個數字不同，且第三個數字是 6 和 8 的公因數。",
  }),
  Object.freeze({
    conditionId: "second_fourth_have_70_as_common_multiple",
    text: "70 是第二個數字和第四個數字的公倍數。",
  }),
  Object.freeze({
    conditionId: "first_digit_double_common_factor",
    text: "第一個數字同時是 22 和 33 的公因數，也是 45 和 60 的公因數。",
  }),
  Object.freeze({
    conditionId: "whole_number_multiple_of_3_and_5",
    text: "這個四位數同時是 3 的倍數和 5 的倍數。",
  }),
  Object.freeze({
    conditionId: "all_digits_distinct",
    text: "四個數字互不重複。",
  }),
]);
...
function statementText(statement, target) {
  switch (statement?.kind) {
    case "contains_one":
      return `1 是 ${target} 的因數。`;
    case "contains_self":
      return `${target} 是 ${target} 的因數。`;
    case "factor_count_even":
      return `${target} 的因數個數是偶數。`;
    default:
      throw new Error(`G5AU02_DISPLAY_STATEMENT_KIND_UNSUPPORTED:${statement?.kind ?? "missing"}`);
  }
}
  ...
  switch (item.patternSpecId) {
    ...
    case "ps_g5a_u02_multi_constraint_digit_code":
      return deepFreeze({
        schemaName: "G5AU02QuestionDisplayModel",
        schemaVersion: 1,
        kind: "source_backed_digit_code_conditions",
        digitCount: 4,
        conditions: clone(SOURCE_PASSWORD_CONDITIONS),
        sourceEvidence: "5a02a1:p2:right-top",
      });
...
function sequenceText(sequence) {
  return sequence.map((entry) => entry.text).join("、");
}
...
export function serializeG5AU02QuestionDisplayModel(basePrompt, model) {
  ...
  switch (model.kind) {
    case "masked_factor_sequence":
      return `補回 ${model.target} 的完整因數表中的缺漏值。\n因數表：${sequenceText(model.sequence)}`;
    ...
    case "symbolic_complete_factor_sequence":
      return `觀察下列完整因數表，求出原數與所有代號。\n因數表：${sequenceText(model.sequence)}\n${model.targetRuleText}`;
    case "factor_list_statement_set":
      return `根據完整因數表，判斷下列敘述是否正確。\n因數表：${model.factorList.join("、")}\n${model.statements.map((row) => `${row.statementNumber}. ${row.text}`).join("\n")}`;
    case "source_backed_digit_code_conditions":
      return `依照下列條件，找出唯一的四位數密碼。\n${model.conditions.map((row, index) => `${index + 1}. ${row.text}`).join("\n")}`;
    default:
      throw new Error(`G5AU02_DISPLAY_KIND_UNSUPPORTED:${model.kind}`);
  }
}
...
export function validateG5AU02QuestionDisplayModel(item, model, promptText = "") {
  ...
  switch (item.patternSpecId) {
    ...
    case "ps_g5a_u02_complete_factor_list_unknown_values":
      ...
      if (model.sequence?.filter((entry) => entry.role === "unknown").length !== data.unknownKeys?.length) {
        errors.push("G5AU02_UNKNOWN_FACTOR_SYMBOL_COUNT_MISMATCH");
      }
    ...
    case "ps_g5a_u02_multi_constraint_digit_code":
      ...
      requireExactArray(
        model.conditions?.map((row) => row.conditionId),
        SOURCE_PASSWORD_CONDITIONS.map((row) => row.conditionId),
        "G5AU02_SOURCE_PASSWORD_CONDITIONS_MISSING",
        errors,
      );
      break;
    default:
      errors.push("G5AU02_PROMPT_COMPLETENESS_PATTERN_UNSUPPORTED");
  }

  if (typeof promptText !== "string" || promptText.length === 0) errors.push("G5AU02_VISIBLE_PROMPT_REQUIRED");
  if (["candidate_selection", "masked_factor_sequence", "symbolic_complete_factor_sequence", "factor_list_statement_set", "source_backed_digit_code_conditions"].includes(model.kind)) {
...
export function enrichG5AU02GeneratedItemPrompt(item) {
  ...
  if (!validation.ok) {
    throw new Error(`G5AU02_VISIBLE_PROMPT_BLOCKED:${validation.errors.join(",")}`);
  }
  return deepFreeze({ prompt, questionDisplayModel });
}

export function getG5AU02PromptCompletenessPatternIds() {
  return [...BLOCKING_PATTERN_IDS];
}
