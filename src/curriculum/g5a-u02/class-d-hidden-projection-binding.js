...
const BOUND_CLASS_D_SPECS = Object.freeze(
  CLASS_D_IDS.map((patternSpecId) => bindSpec(getG5AU02HiddenPatternSpecById(patternSpecId))),
);
const BOUND_BY_ID = new Map(BOUND_CLASS_D_SPECS.map((spec) => [spec.patternSpecId, spec]));

export function getG5AU02BoundClassDSpecs() {
  return BOUND_CLASS_D_SPECS;
}

export function getG5AU02BoundClassDSpecById(patternSpecId) {
...
export function auditG5AU02ClassDHiddenProjectionBinding() {
  ...
  if (JSON.stringify(classDIds) !== JSON.stringify(CLASS_D_IDS)) {
    errors.push("G5AU02_PROJECTION_CLASS_D_ID_MISMATCH");
  }
  ...
  for (const patternSpecId of CLASS_D_IDS) {
    ...
    try {
      ...
      if (binding.templateFamilyIds[0] !== projection.templateFamilyIds[0]) {
        errors.push("G5AU02_PROJECTION_TEMPLATE_FAMILY_MISMATCH");
      }
    } catch (error) {
  ...
  return Object.freeze({
    ...
    unboundClassCCount: classCIds.length,
    boundClassDCount: BOUND_CLASS_D_SPECS.length,
  });
}

export function generateG5AU02ClassDFromHiddenProjection(patternSpecId, options = {}) {
  if (!CLASS_D_SET.has(patternSpecId)) {
...
export function validateG5AU02ClassDFromHiddenProjection(item) {
  ...
  if (!binding || !CLASS_D_SET.has(binding.patternSpecId)) {
