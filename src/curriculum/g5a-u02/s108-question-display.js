...
export function buildG5AU02S108QuestionDisplayModel(item) {
  ...
  return freeze({
    ...
    distributionWitness: clone(data.distributionWitness),
    remainder: {
      role: "student_remainder_response",
      responseText: "______",
      unitLabel,
    },
    publicAnswerPolicy: "known_remainder_given_transfer_reasoning_required",
  });
}
...
export function serializeG5AU02S108QuestionDisplayModel(model) {
  ...
  return [
    ...
    `原分裝：${model.distributionWitness.knownDistribution.equationText}`,
    `改分裝：${model.distributionWitness.transferredDistribution.equationText}`,
...
export function validateG5AU02S108QuestionDisplayModel(item, model, promptText = "") {
  ...
  if (!exact(model.divisorRelation, data.divisorRelation)
    || !exact(model.distributionWitness, data.distributionWitness)
    || model.publicAnswerPolicy !== "known_remainder_given_transfer_reasoning_required") {
  ...
  const requiredVisible = [
    ...
    model.distributionWitness?.knownDistribution?.equationText,
    model.distributionWitness?.transferredDistribution?.equationText,
