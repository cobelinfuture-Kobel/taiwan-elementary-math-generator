import { getG4AU08AdapterContracts } from "./canonical-generated-item-adapter.js";
...
function validateLifecycle(item, errors) {
  ...
  if (!lifecycle || typeof lifecycle !== "object") {
    errors.push(error(ERROR_CODES.LIFECYCLE_INVALID, "lifecycle", "hidden contract object", lifecycle));
    return;
  }
  if (lifecycle.selectorVisibility !== "hidden" || lifecycle.adapterStatus !== "implemented_hidden") {
    errors.push(error(ERROR_CODES.LIFECYCLE_INVALID, "lifecycle", "hidden", lifecycle));
  }
...
export function getG4AU08ValidatorErrorCodes() {
  return ERROR_CODES;
}

export function getG4AU08ValidatorContracts() {
  return CONTRACTS;
}
...
export function validateG4AU08CanonicalItem(item) {
  ...
  if (!contract) {
    errors.push(error(ERROR_CODES.TEMPLATE_UNMAPPED, "legacyTemplateId", "one of 12 mapped templates", item.legacyTemplateId));
  } else {
    validateIdentity(item, contract, errors);
    validateRoles(item, contract, errors);
  }
  ...
  return Object.freeze({
    ...
    validationStage: "validator_contract_rebase",
    arithmeticRecomputation: "deferred_to_existing_runtime_and_s76h_mutation_suite"
  });
}
