...
import {
  auditP03F10PublicSelectorComposition,
  getVisibleBatchAKnowledgePoint,
  getVisiblePatternGroupsForKnowledgePoint,
  listBatchAKnowledgePointAvailabilityBySource,
} from "../../../site/modules/curriculum/registry/batch-a-selector-p03f10-extension.js";
import { validateP03F10PatternDefinition } from "../../../site/modules/curriculum/batch-a/source-pattern-full-product-p03f10-extension.js";
import { buildBatchABrowserPlan } from "../../../site/modules/curriculum/batch-a/batch-a-browser-generator-p03f10.js";
import { generateBatchABrowserQuestions } from "../../../site/modules/curriculum/batch-a/batch-a-browser-question-router.js";
import { validateBatchABrowserPlan, validateBatchABrowserQuestions } from "../../../site/modules/curriculum/batch-a/batch-a-browser-validator-p03f10.js";
...
const PREDECESSOR_READBACK = "docs/curriculum/output/P03F_W3_DIRECT_PRODUCT_VERTICAL_SLICE009_READBACK.md";
export const P03F_SLICE010_PRODUCT_ADMISSION_VERSION = "p03f-slice010-product-admission-v1";
...
function plan() {
  return Object.freeze({
    ...
    selectedKnowledgePointIds: Object.freeze([G4A_U09_HUNDREDTH_DECIMAL_KP_ID]),
    selectedPatternGroupIds: Object.freeze([G4A_U09_HUNDREDTH_DECIMAL_PATTERN_GROUP_ID]),
    questionMode: "numeric",
    questionCount: 8,
    ordering: "groupedByPattern",
    includeAnswerKey: true,
    generationSeed: "p03f-slice010-hundredth-decimal-d0",
    title: "四年級｜百分位與 0.01",
...
function capabilityWitness(question, numberSystem, domainValidator) {
  ...
  const valuePolicy = { allowedMagnitudeClasses: ["DECIMAL_FRACTION"], allowZero: false, maxCanonicalScale: 2, allowedCanonicalScales: [2] };
  const numberResult = numberSystem.execute({ action: "NORMALIZE", knowledgePointId: G4A_U09_HUNDREDTH_DECIMAL_KP_ID, sourceNodeId: G4A_U09_SOURCE_ID, value, assertedCapabilityId: "cap_decimal_number_system" });
  const domainResult = domainValidator.execute({ action: "VALIDATE_VALUE", knowledgePointId: G4A_U09_HUNDREDTH_DECIMAL_KP_ID, sourceNodeId: G4A_U09_SOURCE_ID, value, valuePolicy, assertedCapabilityId: "cap_decimal_domain_validator" });
  return Object.freeze({
...
export function materializeP03FSlice010ProductAdmission() {
  const authority = readJson("slice010-hundredth-decimal-authority.json");
  const manifest = readJson("slice010-product-admission.manifest.json");
  ...
  const selectorProjectionAudit = auditG4AU09HundredthDecimalSelectorProjection();
  const selectorCompositionAudit = auditP03F10PublicSelectorComposition();
  const patternAudit = validateP03F10PatternDefinition();
  const controlAudit = auditFullProductPublicControlProfiles({ includeW3Slice010: true });
  ...
  const publicSource = currentSources.find((row) => row.sourceId === G4A_U09_SOURCE_ID) ?? null;
  const selectorRow = getVisibleBatchAKnowledgePoint(G4A_U09_HUNDREDTH_DECIMAL_KP_ID);
  const visibleGroups = getVisiblePatternGroupsForKnowledgePoint(G4A_U09_HUNDREDTH_DECIMAL_KP_ID);
  const availability = listBatchAKnowledgePointAvailabilityBySource(G4A_U09_SOURCE_ID);
  const controlProfile = getFullProductPublicControlProfile(G4A_U09_SOURCE_ID);
  ...
  const metrics = Object.freeze({
    ...
    cumulativeW3ProductAdmissionCount: 11 + (d0Complete ? 1 : 0),
    remainingDirectSliceCount: 44 - (d0Complete ? 1 : 0),
    remainingDirectKnowledgePointCount: 71 - (d0Complete ? 1 : 0),
  ...
  return Object.freeze({
    ...
    status: manifest.status, version: P03F_SLICE010_PRODUCT_ADMISSION_VERSION, authority: Object.freeze(authority), manifest: Object.freeze(manifest),
    ...
    artifactIntegrity: artifacts, metrics, productAdmissionState: d0Complete ? "PRODUCTION_ADMITTED_D0" : "RUNTIME_CONNECTED_PENDING_CHROMIUM_ACCEPTANCE", d0Complete,
  });
}
