...
export const W03_A00_TASK = 'POSTG-APP-W03-A00_13SourceNodeApplicationCapabilityAssessmentAndAdmissionBaseline';
...
export function validateW02A08R4Approval(loaded) {
  const issues = [];
  const { decision, evidence, claim, registryValidation, productionAccess, publicAccess, w03ShadowAccess } = loaded;
  ...
  if (!decision
      ...
      || decision.controllerTransition?.nextTaskId !== W03_A00_TASK) {
  ...
  if (!evidence
      ...
      || evidence.nextWaveActivation?.nextTaskId !== W03_A00_TASK) {
  ...
  if (!claim
      ...
      || claim.nextStep?.taskId !== W03_A00_TASK) {
  ...
  return {
    ...
    nextShortestStep: issues.length === 0 ? W03_A00_TASK : W02_A08R4_TASK
  };
}
