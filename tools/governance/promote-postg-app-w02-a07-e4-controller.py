...
p=R/'tests/curriculum/postg-app-w01-a06e-operator-approval.test.js';t=p.read_text()
t=one(t,"  assert.equal(w02.humanReviewReady, false);\n",f"  const expectedW02HumanReviewReady = w02.state === '{READY}';\n  assert.equal(w02.humanReviewReady, expectedW02HumanReviewReady);\n  if (expectedW02HumanReviewReady) {{\n    assert.equal(w02.humanReviewPackageComplete, true);\n    assert.equal(w02.reviewDecision, 'NOT_STARTED');\n  }}\n",'w01-monotonic-review');p.write_text(t)
