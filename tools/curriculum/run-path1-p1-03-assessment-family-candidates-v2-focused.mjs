...
const publicBinding = getPath1PublicWorksheetBlock("P1-03");
if (!publicBinding) fail("P1_03_PUBLIC_BINDING_MISSING");
if (publicBinding.diversityProfileId !== PATH1_P1_03_DIVERSITY_PROFILE_ID) {
  fail("P1_03_PUBLIC_BINDING_CHANGED", {
    expected: PATH1_P1_03_DIVERSITY_PROFILE_ID,
    actual: publicBinding.diversityProfileId,
  });
}
...
const publicFamilyMetadata = [...new Set(
  publicResult.worksheetDocument.questions
    .map((entry) => entry.metadata?.path1PatternFamilyId)
    .filter(Boolean),
)].sort();
...
const report = {
  ...
  publicBindingProfileId: publicBinding.diversityProfileId,
  publicFamilyMetadata,
  publicCutoverApproved: false,
  candidateLeakToPublicRoute: false,
