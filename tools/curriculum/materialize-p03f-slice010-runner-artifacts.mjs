...
const TMP_DIR = path.join(ROOT, "tmp/p03f-slice010-product-acceptance");
const OUTPUT_DIR = path.join(ROOT, "docs/curriculum/output/p03f-slice010-product-admission");
const MANIFEST_PATH = path.join(ROOT, "data/curriculum/full-product/p03f/slice010-product-admission.manifest.json");
const FILES = Object.freeze({ html: "g4a-u09-hundredth-decimal.html", pdf: "g4a-u09-hundredth-decimal.pdf", report: "p03f-slice010-product-acceptance-report.json" });
const sha256 = (filePath) => crypto.createHash("sha256").update(fs.readFileSync(filePath)).digest("hex");
const readJson = (filePath) => JSON.parse(fs.readFileSync(filePath, "utf8"));
const writeJson = (filePath, value) => fs.writeFileSync(filePath, `${JSON.stringify(value, null, 2)}\n`);

const manifest = readJson(MANIFEST_PATH);
if (manifest.status !== "SLICE010_RUNTIME_CONNECTED_PENDING_CHROMIUM_ACCEPTANCE") throw new Error(`P03F10_MATERIALIZATION_STATE_INVALID:${manifest.status}`);
for (const fileName of Object.values(FILES)) {
  const sourcePath = path.join(TMP_DIR, fileName);
  if (!fs.existsSync(sourcePath)) throw new Error(`P03F10_MATERIALIZATION_INPUT_MISSING:${fileName}`);
}
...
if (
  ...
  || JSON.stringify(report.decimalValues) !== JSON.stringify(["0.01"])
  || JSON.stringify(report.decimalScales) !== JSON.stringify([2])
  || report.duplicatePromptFindingCount !== 0
  || report.overflowFindingCount !== 0
  || report.consoleErrorCount !== 0
  || report.pageErrorCount !== 0
  || report.semanticScopeFindingCount !== 0
) throw new Error(`P03F10_MATERIALIZATION_REPORT_INVALID:${JSON.stringify(report)}`);
...
if (htmlSha256 !== report.htmlSha256 || pdfSha256 !== report.pdfSha256) throw new Error(`P03F10_MATERIALIZATION_HASH_COPY_MISMATCH:${htmlSha256}:${pdfSha256}`);
const materializedReport = {
  ...report,
  materialization: {
    mode: "GITHUB_ACTIONS_RUNNER_BINARY_COMMIT",
    workflowRunId: Number(process.env.GITHUB_RUN_ID ?? 0) || null,
    sourceHeadSha: process.env.P03F10_SOURCE_HEAD_SHA ?? null,
    visualReviewAuthority: "PENDING_OPERATOR_REVIEW",
  },
};
writeJson(path.join(OUTPUT_DIR, FILES.report), materializedReport);
manifest.status = "SLICE010_ARTIFACT_MATERIALIZED_PENDING_VISUAL_REVIEW";
...
manifest.mainlineBoundary.queuePositionConsumed = 9;
manifest.mainlineBoundary.slice010KnowledgePointAdmitted = false;
writeJson(MANIFEST_PATH, manifest);
console.log(`P03F10_RUNNER_ARTIFACT_MATERIALIZED=${JSON.stringify({ htmlSha256, pdfSha256, workflowRunId: manifest.exactAcceptance.artifactMaterializationWorkflowRunId })}`);
