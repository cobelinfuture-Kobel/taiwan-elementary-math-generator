...
try {
  ...
  const pageMetrics = await page.$$eval(".worksheet-page", (nodes) => nodes.map((node, index) => ({
    ...
    overflowY: node.scrollHeight > node.clientHeight + 1, overflowX: node.scrollWidth > node.clientWidth + 1, className: node.className,
  })));
...
const prompts = evidence.allQuestions.map((row) => row.blankedDisplayText);
...
const semanticScopeFindingCount = evidence.allQuestions.filter((row) => {
  if (/(?:算式|_{2,}|答\s*[:：]|\{\{)/.test(row.blankedDisplayText ?? "")) return true;
  if (row.operationFamilyId === "simplify_fraction") {
    return row.commonFactor <= 1
  ...
  const value = row.sharePerRecipient;
  const quotientValid = value?.numerator * row.recipientCount === row.totalQuantity * value?.denominator;
  const contextValid = row.questionMode !== "application"
    || (row.metadata?.bindingCandidateId === "w02_bind_ps_g5a_u04_quotient_as_fraction_context_share_per_recipient_application"
      && row.metadata?.contextLineage?.atomicEpisodeId === "gctx_episode_crop_batch_plan_direct_quantity");
  return !quotientValid || !contextValid || row.metadata?.applicationClassification !== "APPLICATION_REQUIRED";
}).length;

const report = {
  schemaName: "P03FSlice013ChromiumProductAcceptanceReportV2",
  taskId: evidence.taskId,
  status: "PASS_AUTOMATED_PENDING_VISUAL_REVIEW",
  sourceId: "g5a_u04_5a04",
  knowledgePointIds: evidence.slice.knowledgePointIds,
  questionCount: evidence.allQuestions.length,
  ...
  duplicatePromptFindingCount, overflowFindingCount,
...
fs.writeFileSync(path.join(OUTPUT, "p03f-slice013-product-acceptance-report.json"), `${JSON.stringify(report, null, 2)}\n`);
if (report.questionCount !== 9 || report.answerKeyItemCount !== 9 || report.patternSpecIds.length !== 5
  || report.physicalPdfPageCount !== report.questionPageCount + report.answerKeyPageCount || report.physicalPdfPageCount !== 2
