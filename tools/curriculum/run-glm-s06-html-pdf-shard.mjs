...
function resolvedQuestionLayout(document) {
  ...
  const columns = Number(resolved?.columns);
  const rowsPerPage = Number(resolved?.rowsPerPage);
  return {
    ...
    authority: resolved ? "layoutResolution.resolvedQuestionLayout" : "missing",
  };
}
...
function classifyDocument(result, scenario, resolved) {
  ...
  if (document?.layoutResolution?.layoutExact !== true) return "NOT_EXACT";
  if (document?.layoutResolution?.capped !== false) return "CAPPED";
  if (document?.layoutResolution?.layoutMode !== "exact_approved_matrix") return "LAYOUT_MODE_INVALID";
  if (
...
function buildScenario(scenario) {
  ...
  const documentClassification = classifyDocument(result, scenario, resolved);
  const base = {
    ...clone(scenario),
  ...
  if (documentClassification !== "PASS") return { scenario: base, html: null };
...
function renderFindingCodes(dom, pdf, scenario) {
  ...
  if (pdf.renderedPageCount !== pdf.pdfPageCount) findings.push("PDF_RENDER_PAGE_COUNT_MISMATCH");
  if (pdf.nonblankPdfPageCount !== pdf.pdfPageCount) findings.push("PDF_NONBLANK_PAGE_COUNT_MISMATCH");
  return findings;
}
...
for (const entry of built) {
  ...
  scenario.acceptanceStatus = scenario.documentClassification === "PASS"
    && scenario.renderStatus === "PASS"
    ? "PASS"
    : "FAIL";
...
const acceptancePassCount = built.filter(
  (entry) => entry.scenario.acceptanceStatus === "PASS",
).length;
const manifest = {
  schemaVersion: "glm-s06-html-pdf-shard-v1",
  task: "GLM-S06_270ScenarioPostFixAcceptance",
  status: acceptancePassCount === built.length ? "SHARD_ACCEPTED" : "SHARD_REJECTED",
  shardIndex,
  ...
  acceptancePassCount,
  acceptanceFailureCount: built.length - acceptancePassCount,
...
if (manifest.status !== "SHARD_ACCEPTED") {
  const failures = manifest.scenarios
  ...
  throw new Error(`GLM_S06_SHARD_REJECTED:${JSON.stringify(failures)}`);
}
