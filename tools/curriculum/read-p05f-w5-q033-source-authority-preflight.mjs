...
if(queue.status!=="W5_DIRECT_PRODUCT_VERTICAL_SLICE_QUEUE_FROZEN"||!queue.queueFrozen||!queue.queueRegistryParity) throw new Error("Q033_QUEUE_NOT_FROZEN");
const row=queue.queueEntries.find(x=>x.queuePosition===33);
if(!row) throw new Error("Q033_FROZEN_ROW_MISSING");
if(row.implementationTaskId!=="P05F_W5DirectProductVerticalSlice033Implementation") throw new Error(`Q033_TASK:${row.implementationTaskId}`);
if(row.previousSliceId!=="p05e_q032_r3_g4a_u05_4a05_profile_geometry_property_c1") throw new Error(`Q033_PREDECESSOR:${row.previousSliceId}`);
...
if(!source) throw new Error(`Q033_R02_SOURCE_MISSING:${row.primarySourceNodeId}`);
const candidates=row.knowledgePointIds.map(id=>{
  const candidate=source.candidates.find(x=>x.knowledgePointId===id);
  if(!candidate) throw new Error(`Q033_R02_KP_MISSING:${id}`);
  return candidate;
});
...
const mappings=row.knowledgePointIds.map(id=>{
  ...
  if(!mapping) throw new Error(`Q033_R04_MAPPING_MISSING:${id}`);
  if(mapping.primaryRuntimeProfileId!==row.primaryRuntimeProfileId) throw new Error(`Q033_PROFILE_MISMATCH:${id}`);
  return mapping;
});
const contractOnlyIds=new Set(r04.capabilities.filter(x=>x.deliveryStatus==="contract_only").map(x=>x.capabilityId));
const contractOnlyRequired=[...new Set(mappings.flatMap(x=>x.requiredRuntimeCapabilityIds.filter(id=>contractOnlyIds.has(id))))].sort();
const frozenRequired=[...row.requiredW5CapabilityIds].sort();
if(JSON.stringify(contractOnlyRequired)!==JSON.stringify(frozenRequired)) throw new Error(`Q033_W5_CAPABILITY_MISMATCH:${JSON.stringify({contractOnlyRequired,frozenRequired})}`);
const report={
  schemaName:"P05FW5Q033SourceAuthorityReadbackV1",
  status:"PASS_Q033_EXACT_AUTHORITY_READBACK",
  queue:row,
...
process.stdout.write(`P05F33_PREFLIGHT_READBACK=${JSON.stringify(report)}\n`);
