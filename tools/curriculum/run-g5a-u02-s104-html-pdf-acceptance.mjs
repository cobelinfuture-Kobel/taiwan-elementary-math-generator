...
function buildScenario(patternSpecId, layout, seed, includeAnswerKey) {
  ...
  if (!overlaid?.ok) throw new Error(`S104_LAYOUT_OVERLAY_BLOCKED:${patternSpecId}:${layout.layoutId}:${JSON.stringify(overlaid?.errors ?? [])}`);
  ...
  if (document.layoutResolution?.layoutExact !== true
    ...
    throw new Error(`S104_LAYOUT_RESOLUTION_MISMATCH:${patternSpecId}:${layout.layoutId}`);
  }
  if (document.layoutResolution?.resolvedAnswerLayout?.columns !== ANSWER_COLUMNS
    ...
    throw new Error(`S104_ANSWER_PROFILE_MISMATCH:${patternSpecId}:${layout.layoutId}`);
  }
  return document;
}
...
function relabelPages(pages, layoutId, type) {
  return pages.map((page, index) => ({
    ...
    s104LayoutId: layoutId,
    s104PageType: type,
  }));
}

function combineDocuments(documents, layouts, includeAnswerKey) {
...
try {
  for (const [patternIndex, patternSpecId] of P0_PATTERNS.entries()) {
    const documents = APPROVED_LAYOUTS.map((layout, index) => buildScenario(patternSpecId, layout, 104000 + patternIndex * 100 + index + 1, false));
    const combined = combineDocuments(documents, APPROVED_LAYOUTS, false);
    ...
    for (const includeAnswerKey of [false, true]) {
      const boundaryDocuments = BOUNDARY_LAYOUTS.map((layout, index) => buildScenario(patternSpecId, layout, 204000 + patternIndex * 100 + index + 1, includeAnswerKey));
      const boundaryDocument = combineDocuments(boundaryDocuments, BOUNDARY_LAYOUTS, includeAnswerKey);
