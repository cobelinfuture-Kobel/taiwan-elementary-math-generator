...
const targets=queueRows(queue).map((target)=>{
  const row=matrix.rows.find((candidate)=>candidate.routeIndex===target.routeIndex&&candidate.routeId===target.routeId);
  if(!row)fail("PGC_R08_A03_A01_ROUTE_MISSING",target);
...
try{
  ...
  const routeBindingFailed=failed.filter((row)=>String(row.browserEvidence?.errorCode??"").includes("TARGET_CAPACITY_ROUTE_NOT_BOUND"));
