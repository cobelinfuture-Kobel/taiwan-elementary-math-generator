...
try {
  ...
  const worker = async () => {
    while (true) {
      ...
      const policyBrowser = wrapBrowserWithDisabledCurrentValueSelectionPolicy(exactBrowser, {
        onDisposition: (event) => controlEvents.push(event),
      });
      results[index] = await executeRoute(policyBrowser, row);
    }
  };
  ...
  const sampleEvidence = plan.shards.map((shard) => {
    const row = results.find((candidate) => candidate.routeIndex === shard.firstRouteIndex);
    const first = row?.browserEvidence?.firstGeneration ?? {};
    return {
      shardId: shard.shardId,
      routeId: row?.routeId ?? null,
      sampleHtmlPath: first.sampleHtmlPath ?? null,
      samplePdfPath: first.samplePdfPath ?? null,
      htmlSha256: first.htmlSha256 ?? null,
      pdfSha256: first.pdfSha256 ?? null,
    };
  });
  ...
  const report = {
    ...
    status: failed.length === 0 ? "PASS_ALL_793_LEGAL_ROUTES" : "FAIL_CONVERGED_793_ROUTE_REPLAY",
    summary: {
