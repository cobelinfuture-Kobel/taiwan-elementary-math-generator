...
const OUTPUT = path.join(ROOT, "tmp/p04f-slice003-targeted-route-replay");
...
const options = {
  ...
  questionCount: 8,
  generationSeed: "p04f3-q003-targeted-route-replay",
...
if (!result.ok || !result.worksheetDocument) {
  throw new Error(`P04F3_TARGETED_WORKSHEET_FAILED:${JSON.stringify(result.errors)}`);
}
...
const htmlPath = path.join(OUTPUT, "q003-targeted-route.html");
const pdfPath = path.join(OUTPUT, "q003-targeted-route.pdf");
...
const crossLayerMismatchCount = questions.filter((q, index) => !document.answerKeyItems[index]
  || document.answerKeyItems[index].questionId !== q.id
  || document.answerKeyItems[index].answerText !== q.answerText
...
const directions = [...new Set(questions.map((q) => q.metadata?.conversionDirection))].sort();
const report = {
  schemaName: "P04F3Q003TargetedRouteReplayV1",
  ...
  clockQuestionCount: questions.filter(isClock).length,
  unitQuestionCount: questions.filter(isUnit).length,
...
const pass = report.publicSourceCount === 37
  ...
  && report.clockQuestionCount === 4
  && report.unitQuestionCount === 4
  && report.questionPageCount === 1
  && report.answerPageCount === 1
  && report.renderedWorksheetPageCount === 2
  && report.physicalPdfPageCount === 2
  && expectedDirections.every((direction) => report.directions.includes(direction))
...
if (!pass) throw new Error(`P04F3_TARGETED_ROUTE_REPLAY_FAILED:${JSON.stringify(report)}`);
console.log(`P04F3_TARGETED_ROUTE_REPLAY=${JSON.stringify(report)}`);
