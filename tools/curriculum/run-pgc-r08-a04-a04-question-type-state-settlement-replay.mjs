...
const OUT = path.join(ROOT, "tmp/pgc-r08-a04-a04-question-type-state-settlement");
...
async function waitForServer() {
  ...
  fail("PGC_R08_A04_A04_SITE_SERVER_TIMEOUT");
}
...
const targets = queueRows(queue).map((target) => {
  const row = matrix.rows.find(
    (candidate) => candidate.routeIndex === target.routeIndex && candidate.routeId === target.routeId,
  );
  if (!row) fail("PGC_R08_A04_A04_ROUTE_MISSING", target);
  return enrichBrowserRowWithExactPatternGroups(row);
});
if (targets.length !== PLAN.targetRouteCount) {
  fail("PGC_R08_A04_A04_ROUTE_COUNT_DRIFT", { actual: targets.length });
}
...
try {
  ...
  const uiOptionsFailed = failed.filter((row) => row.gateStatus.UI_OPTIONS_PASS !== "PASS");
  const settlementResidual = uiOptionsFailed.filter((row) =>
    String(row.browserEvidence?.errorCode ?? "").includes("Timeout"),
  );
  const dispositionCounts = Object.fromEntries(
    Object.values(DISABLED_CONTROL_POLICY_CODES).map((code) => [
      code,
      controlEvents.filter((event) => event.disposition === code).length,
    ]),
  );
  const report = {
    schemaName: "PGCR08A04A04QuestionTypeStateSettlementReplayReportV1",
    schemaVersion: 1,
    programId: PLAN.programId,
    taskId: PLAN.taskId,
    status: uiOptionsFailed.length === 0
      ? "PASS_QUESTION_TYPE_STATE_SETTLEMENT_9_OF_9"
      : "FAIL_QUESTION_TYPE_STATE_SETTLEMENT_NONZERO",
    summary: {
      targetRouteCount: targets.length,
      terminalRouteCount: results.length,
      uiOptionsPassCount: results.filter((row) => row.gateStatus.UI_OPTIONS_PASS === "PASS").length,
      questionTypeStateSettlementResidualCount: settlementResidual.length,
      fullNineGatePassCount: results.filter((row) =>
        GATE_CODES.every((gate) => row.gateStatus[gate] === "PASS")
      ).length,
      downstreamFailCount: failed.length - uiOptionsFailed.length,
  ...
  if (
    ...
    report.summary.uiOptionsPassCount !== PLAN.acceptance.uiOptionsPassCount ||
    report.summary.questionTypeStateSettlementResidualCount !== 0 ||
    report.summary.DISABLED_VALUE_MISMATCH !== 0
  ) {
    fail("PGC_R08_A04_A04_ACCEPTANCE_FAILED", report.summary);
  }
} finally {
