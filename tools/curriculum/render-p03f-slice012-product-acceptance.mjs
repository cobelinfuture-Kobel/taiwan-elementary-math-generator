...
const OUTPUT = path.join(ROOT, "tmp/p03f-slice012-product-acceptance");
fs.mkdirSync(OUTPUT, { recursive: true });
const evidence = materializeP03FSlice012ProductAdmission();
if (!evidence.predecessorPassed) throw new Error("P03F12_PREDECESSOR_NOT_D0");
if (!evidence.worksheet.ok || !evidence.document || !evidence.html) throw new Error("P03F12_HTML_INPUT_INVALID");

const htmlPath = path.join(OUTPUT, "g4b-u08-equivalence-cross-product.html");
const pdfPath = path.join(OUTPUT, "g4b-u08-equivalence-cross-product.pdf");
...
const semanticScopeFindingCount = evidence.generation.questions.filter((row) => (
  row.leftNumerator * row.rightDenominator !== row.leftCrossProduct
  || row.rightNumerator * row.leftDenominator !== row.rightCrossProduct
  || row.equivalent !== (row.leftCrossProduct === row.rightCrossProduct)
...
const report = {
  schemaName: "P03FSlice012ChromiumProductAcceptanceReportV1",
  ...
  patternSpecIds: ["ps_g4b_u08_equivalence_cross_product_equivalent_numeric"],
...
fs.writeFileSync(
  path.join(OUTPUT, "p03f-slice012-product-acceptance-report.json"),
  `${JSON.stringify(report, null, 2)}\n`,
);
if (report.questionCount !== 8
  ...
  throw new Error(`P03F12_CHROMIUM_ACCEPTANCE_FAILED:${JSON.stringify(report)}`);
}
console.log(`P03F12_CHROMIUM_ACCEPTANCE=${JSON.stringify(report)}`);
