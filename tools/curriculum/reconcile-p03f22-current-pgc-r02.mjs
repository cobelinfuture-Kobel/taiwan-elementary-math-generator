...
if (!priorPath || !fs.existsSync(priorPath)) throw new Error("P03F22_PRIOR_PGC_R02_CONTRACT_REQUIRED");
...
const reconciled = { ...current, ...terminalMetadata, selectorAuthority: "batch-a-selector-p03f22-extension.js", bindings };
fs.writeFileSync(CONTRACT, `${JSON.stringify(reconciled, null, 2)}\n`);
console.log(`P03F22_PGC_R02_RECONCILED=${JSON.stringify({ status: reconciled.status, publicSourceCount: reconciled.summary.publicSourceCount, visibleKnowledgePointCount: reconciled.summary.visibleKnowledgePointCount, bindingCount: bindings.length, preservedTerminalKeys: Object.keys(terminalMetadata) })}`);
