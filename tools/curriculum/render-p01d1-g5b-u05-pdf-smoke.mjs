...
import { G5B_U05_SOURCE_ID } from "../../site/modules/curriculum/registry/g5b-u05-selector-projection.js";

const MODULE_DIR = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(MODULE_DIR, "../..");
const OUTPUT_DIR = path.join(ROOT, "tmp/p01d1-g5b-u05-pdf-smoke");
const HTML_PATH = path.join(OUTPUT_DIR, "g5b-u05-large-number.html");
const PDF_PATH = path.join(OUTPUT_DIR, "g5b-u05-large-number.pdf");
...
const worksheetResult = buildBatchABrowserWorksheetDocument({
  ...
  generationSeed: "p01d1-chromium-pdf",
  title: "五年級｜億以上大數練習",
  printLayout: {
    paperSize: "A4",
    columns: 2,
...
if (!worksheetResult.ok || !worksheetResult.worksheetDocument) {
  throw new Error(`P01D1_WORKSHEET_BUILD_FAILED:${JSON.stringify(worksheetResult.errors ?? [])}`);
}

const document = worksheetResult.worksheetDocument;
if (document.generatedQuestions.length !== 16 || document.answerKeyItems.length !== 16) {
  throw new Error(`P01D1_WORKSHEET_COUNT_INVALID:${document.generatedQuestions.length}:${document.answerKeyItems.length}`);
}
...
.worksheet-cell__prompt { font-size: 14pt; line-height: 1.55; overflow-wrap: anywhere; }
.worksheet-cell__answer { margin-top: 3mm; font-size: 12pt; line-height: 1.45; overflow-wrap: anywhere; }
</style>`;
...
try {
  ...
  if (pageMetrics.length !== document.questionPages.length + document.answerKeyPages.length) {
    throw new Error(`P01D1_PAGE_COUNT_MISMATCH:${pageMetrics.length}:${document.questionPages.length + document.answerKeyPages.length}`);
  }
  if (overflowFindings.length > 0) throw new Error(`P01D1_PAGE_OVERFLOW:${JSON.stringify(overflowFindings)}`);
  if (consoleErrors.length > 0 || pageErrors.length > 0) throw new Error(`P01D1_BROWSER_ERRORS:${JSON.stringify({ consoleErrors, pageErrors })}`);
  ...
  if (pdfSizeBytes < 10000) throw new Error(`P01D1_PDF_TOO_SMALL:${pdfSizeBytes}`);

  const report = {
    schemaName: "P01D1G5BU05ChromiumPdfSmokeReportV1",
  ...
  process.stdout.write(`P01D1_CHROMIUM_PDF_READBACK=${JSON.stringify(report)}\n`);
} finally {
  await browser.close();
}
