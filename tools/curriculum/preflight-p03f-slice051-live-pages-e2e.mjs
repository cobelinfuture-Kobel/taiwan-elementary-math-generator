...
const ASSETS = [
  ...
  ["site/modules/curriculum/batch-a/batch-a-browser-generator-p03f51.js", "modules/curriculum/batch-a/batch-a-browser-generator-p03f51.js"],
  ["site/modules/curriculum/batch-a/batch-a-browser-question-router-p03f51.js", "modules/curriculum/batch-a/batch-a-browser-question-router-p03f51.js"],
  ["site/modules/curriculum/batch-a/batch-a-browser-validator-p03f51.js", "modules/curriculum/batch-a/batch-a-browser-validator-p03f51.js"],
  ["site/modules/curriculum/batch-a/batch-a-browser-worksheet-p03f51-extension.js", "modules/curriculum/batch-a/batch-a-browser-worksheet-p03f51-extension.js"],
  ...
  ["site/modules/curriculum/batch-a/source-pattern-full-product-p03f49-extension.js", "modules/curriculum/batch-a/source-pattern-full-product-p03f49-extension.js"],
  ["site/modules/curriculum/batch-a/source-pattern-full-product-p03f51-extension.js", "modules/curriculum/batch-a/source-pattern-full-product-p03f51-extension.js"],
  ...
  ["site/modules/curriculum/registry/batch-a-selector-p03f51-extension.js", "modules/curriculum/registry/batch-a-selector-p03f51-extension.js"],
...
async function waitForExactDependencyClosure() {
  ...
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    try {
      ...
      for (const [repoPath, publicPath] of ASSETS) {
        ...
        url.searchParams.set("p03f51-preflight", expectedSha256.slice(0, 16));
  ...
  throw new Error(`P03F51_DEPENDENCY_CLOSURE_NOT_EXACT:${lastFailure?.message ?? "unknown"}`);
}
...
const report = {
  schemaName: "P03FSlice051LivePagesPreflightV1",
  status: "PENDING",
...
try {
  ...
  if (!response?.ok()) throw new Error(`P03F51_PREFLIGHT_PAGE:${response?.status() ?? "NO_RESPONSE"}`);
  ...
  report.ui = await page.evaluate(async ({ sourceId, kp }) => {
    ...
    registryUrl.searchParams.set("p03f51-preflight", String(Date.now()));
  ...
  }, { sourceId: G6A_U04_P03F51_SOURCE_ID, kp: G6A_U04_P03F51_KP_ID });
  const ui = report.ui;
  const pass = report.dependencyClosure.assets.length === ASSETS.length
    && ui.sourceOptions.some((option) => option.value === G6A_U04_P03F51_SOURCE_ID)
    && ui.sourceId === G6A_U04_P03F51_SOURCE_ID
  ...
  report.status = pass ? "PASS_P03F51_DEPLOYED_DEPENDENCY_AND_UI_PREFLIGHT" : "FAIL_P03F51_DEPLOYED_DEPENDENCY_AND_UI_PREFLIGHT";
  if (!pass) throw new Error(`P03F51_PREFLIGHT_UI:${JSON.stringify(report.ui)}`);
} catch (error) {
  report.status = "FAIL_P03F51_DEPLOYED_DEPENDENCY_AND_UI_PREFLIGHT";
  report.error = String(error.message ?? error);
  throw error;
} finally {
  if (browser) await browser.close();
  writeFileSync(resolve(OUT, "p03f-slice051-live-pages-preflight-report.json"), `${JSON.stringify(report, null, 2)}\n`);
  console.log(`P03F51_LIVE_PREFLIGHT=${JSON.stringify(report)}`);
}
