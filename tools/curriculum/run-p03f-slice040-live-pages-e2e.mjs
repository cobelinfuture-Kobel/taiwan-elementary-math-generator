...
const DEFAULT_OUTPUT_DIR = resolve(ROOT, "tmp/p03f-slice040-live-pages-e2e");
const EXACT_MERGE_SHA = "74690ae4aa57082c4e0295e7ed62ce5baf89c437";
const EXACT_PAGES_RUN_ID = 31945388801;
const SOURCE_ID = "g5b_u06_5b06";
const KP_ID = "kp_g5b_u06_integer_division_decimal_quotient";
const GROUP_ID = "pg_g5b_u06_integer_division_decimal_quotient_numeric";
const SPEC_ID = "ps_g5b_u06_integer_division_decimal_quotient_numeric";
const QUESTION_COUNT = 24;
const GENERATION_SEED = "p03f40-postmerge-pages-e2e";

const assetContracts = [
  {
    repoPath: "site/modules/curriculum/registry/g5b-u06-rank9-integer-division-decimal-quotient-selector-projection-p03f40.js",
    publicPath: "modules/curriculum/registry/g5b-u06-rank9-integer-division-decimal-quotient-selector-projection-p03f40.js",
  ...
  {
    repoPath: "site/modules/curriculum/public/public-ui-capability-binding-p03f40.js",
    publicPath: "modules/curriculum/public/public-ui-capability-binding-p03f40.js",
    requiredTokens: ["public-ui-capability-binding-p03f39.js", "g5b-u06-rank9-integer-division-decimal-quotient-selector-projection-p03f40.js", "STRUCTURAL_FALLBACK_AVAILABLE"],
  },
  {
  ...
  },
  {
    ...
    requiredTokens: ["batch-a-selector-p03f40-extension.js", "G5B_U06_SOURCE_ID", "LATEST_FIRST_QUERY_SELECTOR_SOURCE_IDS"],
  },
  {
    repoPath: "site/assets/browser/public-capability-ui.js",
    publicPath: "assets/browser/public-capability-ui.js",
    requiredTokens: ["public-ui-capability-binding-p03f40.js"],
  },
  {
    ...
    requiredTokens: ["batch-a-browser-worksheet-p03f40-extension.js", "requestsP03F40"],
  },
];
...
function validateRenderedPair(prompt, answerText) {
  const match = String(prompt).trim().match(/^(\d+)\s*÷\s*(\d+)\s*=\s*[？?]$/);
  if (!match) return { family: "unrecognized", dividend: null, divisor: null, magnitude: "unrecognized", expected: null, actual: extractNumericAnswer(answerText), ok: false };
  const dividend = BigInt(match[1]);
  const divisor = BigInt(match[2]);
  let expected = null;
  try { expected = canonicalTerminatingDecimal(dividend, divisor); } catch { expected = null; }
  const actual = extractNumericAnswer(answerText);
  const magnitude = dividend < divisor ? "below_one" : "above_one";
  return {
    family: "integer_division_decimal_quotient",
    dividend: match[1],
    divisor: match[2],
    magnitude,
    expected,
    actual,
    ok: Boolean(expected && expected.includes(".") && actual === expected),
  };
}

const baseUrl = new URL(argument("base-url", process.env.P03F40_BASE_URL ?? DEFAULT_BASE_URL));
const outputDir = resolve(argument("output-dir", process.env.P03F40_OUTPUT_DIR ?? DEFAULT_OUTPUT_DIR));
const deploymentRetryCount = Number(argument("deployment-retries", process.env.P03F40_DEPLOYMENT_RETRIES ?? "20"));
const deploymentRetryDelayMs = Number(argument("deployment-retry-delay-ms", process.env.P03F40_DEPLOYMENT_RETRY_DELAY_MS ?? "15000"));
...
async function waitForExactDeployment() {
  ...
  for (let attempt = 1; attempt <= deploymentRetryCount; attempt += 1) {
    try {
      ...
      for (const contract of assetContracts) {
        ...
        assetUrl.searchParams.set("p03f40-sha", expectedSha256.slice(0, 16));
  ...
  throw new Error(`Slice040 exact Pages deployment not observed: ${lastFailure?.message ?? "unknown"}`);
}
...
try {
  ...
  const selectorOk = selectorState.sourceMatches
    ...
    && selectorState.availabilitySummary.includes("本單元可選知識點：1")
    && selectorState.availabilitySummary.includes("全部可選：238");
  if (!selectorOk) throw new Error(`Slice040 public selector binding mismatch: ${JSON.stringify(selectorState)}`);
  ...
  if (!generationState.statusText.includes(`已產生 ${QUESTION_COUNT} 題`)
    ...
    throw new Error(`Slice040 live generation/validator failed: ${JSON.stringify(generationState)}`);
  }

  const frameElement = await page.locator("#preview-frame").elementHandle();
  const frame = await frameElement?.contentFrame();
  if (!frame) throw new Error("Slice040 preview iframe did not expose a content frame");
  ...
  const semanticScopeFindingCount = answerChecks.filter((row) => row.family !== "integer_division_decimal_quotient").length;
  const belowOneWitnessCount = answerChecks.filter((row) => row.magnitude === "below_one").length;
  const aboveOneWitnessCount = answerChecks.filter((row) => row.magnitude === "above_one").length;
  ...
  if (output.questionCount !== QUESTION_COUNT
    ...
    || belowOneWitnessCount === 0
    || aboveOneWitnessCount === 0
    || duplicatePromptCount !== 0
    || internalIdLeakage.length !== 0) {
    throw new Error(`Slice040 live worksheet/answer mismatch: ${JSON.stringify({ exactAnswerMismatchCount, semanticScopeFindingCount, belowOneWitnessCount, aboveOneWitnessCount, duplicatePromptCount, internalIdLeakage, answerChecks })}`);
  }

  await frame.evaluate(() => {
    window.__P03F40_PRINT_INVOKED__ = 0;
    window.print = () => { window.__P03F40_PRINT_INVOKED__ += 1; };
  });
  await page.locator("#print-button").click();
  const printInvocationCount = await frame.evaluate(() => window.__P03F40_PRINT_INVOKED__ ?? 0);
  if (printInvocationCount !== 1) throw new Error(`Slice040 print action did not reach preview frame: ${printInvocationCount}`);

  await page.screenshot({ path: resolve(outputDir, "P03F40_LIVE_PAGES_UI.png"), fullPage: true });
  await frame.locator(".worksheet-document").screenshot({ path: resolve(outputDir, "P03F40_LIVE_PAGES_WORKSHEET.png") });
  writeFileSync(resolve(outputDir, "P03F40_LIVE_PAGES_WORKSHEET.html"), await frame.content());

  if (consoleErrors.length || pageErrors.length || requestFailures.length || serverErrors.length) {
    throw new Error(`Slice040 browser diagnostics failed: ${JSON.stringify({ consoleErrors, pageErrors, requestFailures, serverErrors })}`);
  }

  const report = {
    schemaName: "P03FSlice040PostMergeMainPagesE2EV1",
    taskId: "P03F_W3DirectProductVerticalSlice040_PostMergeMainPagesE2E",
    ...
    worksheet: {
      ...
      belowOneWitnessCount,
      aboveOneWitnessCount,
    ...
    forbiddenScope: {
      ...
      decimalDividendExpansion: false,
      estimationExpansion: false,
      zeroPlaceholderExpansion: false,
      siblingKnowledgePointPromotion: false,
      slice041Started: false,
    },
  };
  writeFileSync(resolve(outputDir, "P03F40_POSTMERGE_MAIN_PAGES_E2E.json"), `${JSON.stringify(report, null, 2)}\n`);
  console.log(`P03F40_POSTMERGE_MAIN_PAGES_E2E=${JSON.stringify(report)}`);
} finally {
