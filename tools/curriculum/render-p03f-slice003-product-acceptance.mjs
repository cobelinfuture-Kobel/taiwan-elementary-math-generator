...
const OUTPUT = path.join(ROOT, "tmp/p03f-slice003-product-acceptance");
fs.mkdirSync(OUTPUT, { recursive: true });
const evidence = materializeP03FSlice003ProductAdmission();
if (!evidence.predecessorPassed) throw new Error("P03F3_PREDECESSOR_NOT_D0");
if (!evidence.worksheet.ok || !evidence.document || !evidence.html) throw new Error("P03F3_HTML_INPUT_INVALID");
const htmlPath = path.join(OUTPUT, "g3b-u07-quotient-as-fraction.html");
const pdfPath = path.join(OUTPUT, "g3b-u07-quotient-as-fraction.pdf");
...
const semanticScopeFindingCount = evidence.generation.questions.filter((row) => row.answerText !== `${row.dividend}/${row.divisor}` || row.questionMode !== "numeric" || /(?:算式|_{2,}|答\s*[:：]|\{\{)/.test(row.blankedDisplayText ?? "")).length;
const report = {
  schemaName: "P03FSlice003ChromiumProductAcceptanceReportV1",
  ...
  magnitudeClasses: [...new Set(evidence.generation.questions.map((row) => row.metadata?.magnitudeClass))].sort(),
...
fs.writeFileSync(path.join(OUTPUT, "p03f-slice003-product-acceptance-report.json"), JSON.stringify(report, null, 2) + "\n");
if (
...
) throw new Error(`P03F3_CHROMIUM_ACCEPTANCE_FAILED:${JSON.stringify(report)}`);
console.log(`P03F3_CHROMIUM_ACCEPTANCE=${JSON.stringify(report)}`);
