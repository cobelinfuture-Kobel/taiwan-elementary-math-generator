...
const marker = "PGC-R05 D0 terminal-status compatibility V1";
const D0_STATUS = "PASS_R05_D0_ALL_LEGAL_APPLICATION_ROUTES_CONFORMANT";
...
function patchSource(before) {
  ...
  source = source.replaceAll(
    ...
    `/^PASS_R05_(?:\\d+_OF_\\d+_LIVE_APPLICATION_ROUTES_CONFORMANT|ALL_LIVE_APPLICATION_ROUTES_CONFORMANT_PENDING_CONTRACT_RECONCILIATION|D0_ALL_LEGAL_APPLICATION_ROUTES_CONFORMANT)$/`,
  );
  source = source.replaceAll(
    ...
    `/^(PASS_R05_\\d+_OF_211_LIVE_APPLICATION_ROUTES_CONFORMANT|PASS_R05_ALL_LIVE_APPLICATION_ROUTES_CONFORMANT_PENDING_CONTRACT_RECONCILIATION|PASS_R05_D0_ALL_LEGAL_APPLICATION_ROUTES_CONFORMANT)$/`,
  );
  source = source.replaceAll(
    `assert.equal(report.status, "PASS_R05_ALL_LIVE_APPLICATION_ROUTES_CONFORMANT_PENDING_CONTRACT_RECONCILIATION");`,
    `assert.match(report.status, /^(PASS_R05_ALL_LIVE_APPLICATION_ROUTES_CONFORMANT_PENDING_CONTRACT_RECONCILIATION|PASS_R05_D0_ALL_LEGAL_APPLICATION_ROUTES_CONFORMANT)$/);`,
  );
  source = source.replaceAll(
