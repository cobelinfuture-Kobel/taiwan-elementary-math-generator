...
const SOURCE="g6a_u08_6a08",TARGET="kp_speed_distance_time_relation",COUNT=12;
const PORT=Number(process.env.P07F23_SITE_PORT||"4433"),REMOTE=process.env.P07F23_SITE_URL||null,BASE=REMOTE||("http://127.0.0.1:"+PORT+"/index.html"),
  BASE_ORIGIN=new URL(BASE).origin,CACHE_TOKEN=process.env.P07F23_LIVE_CACHE_TOKEN||String(Date.now()),OUT=path.resolve("tmp/p07f-w7-slice023-classic-ui-acceptance");
...
async function ready(){let last;for(let i=0;i<50;i++){try{const r=await fetch(BASE,{cache:"no-store"});if(r.ok)return;}catch(e){last=e;}await sleep(250);}throw new Error("P07F23_SITE_NOT_READY:"+(last?.message||"unknown"));}
const errors={console:[],page:[],request:[],http:[]},moduleResponses=[];
async function run(){
  const page=await browser.newPage({viewport:{width:1440,height:1100},deviceScaleFactor:1});
  if(REMOTE)await page.route("**/*",async route=>{const request=route.request(),url=new URL(request.url());if(request.method()==="GET"&&url.origin===BASE_ORIGIN&&(url.pathname.endsWith(".js")||url.pathname.endsWith(".css")||url.pathname.includes("/modules/"))){url.searchParams.set("p07f23-e6",CACHE_TOKEN);await route.continue({url:url.href,headers:{...request.headers(),"cache-control":"no-cache","pragma":"no-cache"}});return;}await route.continue();});
  ...
  const url=new URL(BASE);url.searchParams.set("p07f23",String(Date.now()));const response=await page.goto(url.href,{waitUntil:"networkidle",timeout:120000});if(!response?.ok())throw new Error("P07F23_MAIN_HTTP:"+(response?.status()||"none"));
  ...
  if(selector.sourceId!==SOURCE||!selector.targetPresent||!selector.sameUnitDisabled||selector.visibleIds.filter(id=>id.startsWith("kp_speed_")).length!==1)throw new Error("P07F23_SELECTOR:"+JSON.stringify(selector));
  ...
  await page.fill("#generation-seed-input","p07f23-speed-ui");await page.dispatchEvent("#generation-seed-input","change");
  ...
  if(!state.status.includes("已產生 "+COUNT+" 題")||state.tone!=="success"||state.valid!=="false"||state.preview<=0||state.printDisabled)throw new Error("P07F23_GENERATION:"+JSON.stringify(state));
  const frame=await (await page.locator("#preview-frame").elementHandle())?.contentFrame();if(!frame)throw new Error("P07F23_PREVIEW_FRAME_MISSING");await frame.waitForSelector(".worksheet-document",{timeout:30000});
  const worksheet=await frame.evaluate(()=>{const q=[...document.querySelectorAll(".worksheet-cell--question")],a=[...document.querySelectorAll(".worksheet-cell--answer-key")],
    ...
    return{questions:q.length,answers:a.length,overflow,text,leaks:text.includes("kp_speed_")||text.includes("ps_g6a_u08_")||text.includes("P07F23")};});
  if(worksheet.questions!==COUNT||worksheet.answers!==COUNT||worksheet.overflow!==0||worksheet.leaks)throw new Error("P07F23_WORKSHEET:"+JSON.stringify({questions:worksheet.questions,answers:worksheet.answers,overflow:worksheet.overflow,leaks:worksheet.leaks}));
  if(!worksheet.text.includes("固定速率"))throw new Error("P07F23_SEMANTICS_MISSING");
  for(const token of ["平均速率","追趕","相遇","順流","逆流","風速","換算成"])if(worksheet.text.includes(token))throw new Error("P07F23_FORBIDDEN_SEMANTIC_LEAK:"+token);
  await frame.evaluate(()=>{window.__P07F23_PRINT__=0;window.print=()=>window.__P07F23_PRINT__++;});await page.locator("#print-button").click();
  const printCount=await frame.evaluate(()=>window.__P07F23_PRINT__||0);if(printCount!==1)throw new Error("P07F23_PRINT:"+printCount);
  await frame.locator(".worksheet-document").screenshot({path:path.join(OUT,"q023-worksheet.png"),fullPage:true});await page.screenshot({path:path.join(OUT,"q023-ui.png"),fullPage:true});await page.close();
...
try{
  await ready();browser=await chromium.launch({headless:true});const target=await run();if(Object.values(errors).some(x=>x.length))throw new Error("P07F23_BROWSER_DIAGNOSTICS:"+JSON.stringify(errors));
  const report={schemaName:"P07FW7Q023ClassicUIAcceptanceV1",taskId:"P07F_W7DirectProductVerticalSlice023Implementation",status:"PASS_P07F_W7_Q023_CLASSIC_UI_ACCEPTANCE",
    ...
    semanticInvariants:{speedDistanceTimeRelationOwned:true,distanceEqualsSpeedTimesTime:true,compatibleUnits:true,answerBackSubstitution:true,
      speedUnitConversionProtected:true,averageSpeedProtected:true,relativeSpeedProtected:true,effectiveSpeedProtected:true,sameUnitMixedModeFailClosed:true},
    forbiddenScope:{speedUnitConversion:false,averageSpeed:false,relativeSpeedMeetingChasing:false,effectiveSpeedCurrentWind:false,sameUnitMixed:false,crossUnitMixed:false,fullRepositoryRegression:false,globalBrowserReplay:false}};
  writeFileSync(path.join(OUT,"report.json"),JSON.stringify(report,null,2)+"\n");console.log("P07F23_CLASSIC_UI_ACCEPTANCE="+JSON.stringify(report));
}catch(error){writeFileSync(path.join(OUT,"failure.json"),JSON.stringify({schemaName:"P07FW7Q023ClassicUIAcceptanceFailureV1",status:"FAIL",baseUrl:BASE,error:String(error?.stack||error),browser:errors,moduleResponses,
  cacheStrategy:{remote:Boolean(REMOTE),token:CACHE_TOKEN,moduleQueryParam:"p07f23-e6"},server:{stdout:serverOut,stderr:serverErr}},null,2)+"\n");throw error;}
