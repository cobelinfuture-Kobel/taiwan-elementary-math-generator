...
const OUTPUT = path.join(ROOT, "tmp/p03f-slice010-product-acceptance");
fs.mkdirSync(OUTPUT, { recursive: true });
const evidence = materializeP03FSlice010ProductAdmission();
if (!evidence.predecessorPassed) throw new Error("P03F10_PREDECESSOR_NOT_D0");
if (!evidence.worksheet.ok || !evidence.document || !evidence.html) throw new Error("P03F10_HTML_INPUT_INVALID");
const htmlPath = path.join(OUTPUT, "g4a-u09-hundredth-decimal.html");
const pdfPath = path.join(OUTPUT, "g4a-u09-hundredth-decimal.pdf");
...
const semanticScopeFindingCount = evidence.generation.questions.filter((row) => (
  ...
  || row.whole !== 0
  || row.fractionalUnits !== 1
  || row.placeUnit !== "0.01"
  || row.decimalValue !== "0.01"
  || row.answerText !== "0.01"
...
const report = {
  schemaName: "P03FSlice010ChromiumProductAcceptanceReportV1",
  ...
  decimalValues: [...new Set(evidence.generation.questions.map((row) => row.decimalValue))],
  decimalScales: [...new Set(evidence.generation.questions.map((row) => row.finalAnswer?.scale))],
  canonicalDecimalIdentities: evidence.capabilityWitnesses.map((row) => row.domainCanonicalIdentity).filter(Boolean),
...
fs.writeFileSync(path.join(OUTPUT, "p03f-slice010-product-acceptance-report.json"), JSON.stringify(report, null, 2) + "\n");
if (report.questionCount !== 8 || report.answerKeyItemCount !== 8 || report.knowledgePointIds.length !== 1 || report.patternSpecIds.length !== 1 || report.patternAllocation.length !== 1 || report.patternAllocation[0].questionCount !== 8 || JSON.stringify(report.decimalValues) !== JSON.stringify(["0.01"]) || JSON.stringify(report.decimalScales) !== JSON.stringify([2]) || report.canonicalDecimalIdentities.length !== 8 || report.canonicalDecimalIdentities.some((value) => value !== "1e-2") || report.duplicatePromptFindingCount !== 0 || report.physicalPdfPageCount !== report.questionPageCount + report.answerKeyPageCount || report.physicalPdfPageCount !== 2 || report.overflowFindingCount !== 0 || report.consoleErrorCount !== 0 || report.pageErrorCount !== 0 || report.semanticScopeFindingCount !== 0) throw new Error(`P03F10_CHROMIUM_ACCEPTANCE_FAILED:${JSON.stringify(report)}`);
console.log(`P03F10_CHROMIUM_ACCEPTANCE=${JSON.stringify(report)}`);
