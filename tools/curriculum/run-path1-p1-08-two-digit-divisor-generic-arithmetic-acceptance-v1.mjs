...
try {
  ...
  const witness = await page.evaluate(async ({ primaryKps, patterns }) => {
    ...
    const summarize = (count) => {
      ...
      const noFallbackOutsideP108Patterns = questions.every((question) => patterns.includes(question.patternSpecId));
      const sourceAuthorityOk = questions.every((question) => question.sourceId === "g4a_u04_4a04");
      return {
        count,
        ok: result.ok,
  ...
  }, { primaryKps: PRIMARY_KPS, patterns: PATTERNS });
  ...
  invariant(one.errors[0].questionCount === 1, "PATH1_P108_BROWSER_COUNT1_VALUE_MISMATCH", one);
  ...
  invariant(JSON.stringify(twenty.patternCounts) === JSON.stringify([7, 7, 6]), "PATH1_P108_BROWSER_COUNT20_PATTERN_ALLOCATION_MISMATCH", twenty);
  invariant(twenty.missingKnowledgePointMetadata === 20, "PATH1_P108_BROWSER_COUNT20_KP_METADATA_DIAGNOSIS_MISMATCH", twenty);
  invariant(twenty.arithmeticInvariantOk === true, "PATH1_P108_BROWSER_COUNT20_ARITHMETIC_INVARIANT_FAILED", twenty);
  invariant(twenty.noFallbackOutsideP108Patterns === true, "PATH1_P108_BROWSER_COUNT20_PATTERN_FALLBACK", twenty);
  invariant(twenty.sourceAuthorityOk === true, "PATH1_P108_BROWSER_COUNT20_SOURCE_AUTHORITY_MISMATCH", twenty);
  invariant(twenty.routeSelectionMatchesPrimaryKps === true, "PATH1_P108_BROWSER_COUNT20_ROUTE_SELECTION_MISMATCH", twenty);
  ...
  invariant(JSON.stringify(oneTwenty.patternCounts) === JSON.stringify([40, 40, 40]), "PATH1_P108_BROWSER_COUNT120_PATTERN_ALLOCATION_MISMATCH", oneTwenty);
  invariant(oneTwenty.missingKnowledgePointMetadata === 120, "PATH1_P108_BROWSER_COUNT120_KP_METADATA_DIAGNOSIS_MISMATCH", oneTwenty);
  invariant(oneTwenty.arithmeticInvariantOk === true, "PATH1_P108_BROWSER_COUNT120_ARITHMETIC_INVARIANT_FAILED", oneTwenty);
  invariant(oneTwenty.noFallbackOutsideP108Patterns === true, "PATH1_P108_BROWSER_COUNT120_PATTERN_FALLBACK", oneTwenty);
  invariant(oneTwenty.sourceAuthorityOk === true, "PATH1_P108_BROWSER_COUNT120_SOURCE_AUTHORITY_MISMATCH", oneTwenty);
  invariant(oneTwenty.routeSelectionMatchesPrimaryKps === true, "PATH1_P108_BROWSER_COUNT120_ROUTE_SELECTION_MISMATCH", oneTwenty);
  ...
  process.stdout.write(`${JSON.stringify({
    schemaName: "Path1P108TwoDigitDivisorGenericArithmeticBrowserDiagnosticAcceptanceV1",
    status: "PASS_WITH_ISOLATED_ACCEPTANCE_GAPS",
    path1Url: PATH1_URL,
    witness,
    pageErrors,
    consoleErrors,
