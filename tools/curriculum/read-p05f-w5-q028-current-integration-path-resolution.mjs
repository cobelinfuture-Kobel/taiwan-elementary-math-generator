...
const resolution = JSON.parse(fs.readFileSync(path.join(ROOT, "data/curriculum/full-product/p05f/q028-g5a-u10a-solid-net-current-integration-path-resolution.json"), "utf8"));
const queue = materializeP05EW5DirectProductVerticalSliceQueue();
const row = queue.queueEntries.find((entry) => entry.queuePosition === 28);
if (!row) throw new Error("P05F28_QUEUE_ROW_MISSING");
if (row.sliceId !== resolution.queueAuthority.sliceId) throw new Error("P05F28_SLICE_ID_MISMATCH");
if (row.primarySourceNodeId !== resolution.queueAuthority.primarySourceNodeId) throw new Error("P05F28_SOURCE_ID_MISMATCH");
if (row.primaryRuntimeProfileId !== resolution.queueAuthority.primaryRuntimeProfileId) throw new Error("P05F28_RUNTIME_PROFILE_MISMATCH");
if (JSON.stringify(row.knowledgePointIds) !== JSON.stringify(resolution.queueAuthority.knowledgePointIds)) throw new Error("P05F28_KP_MISMATCH");
