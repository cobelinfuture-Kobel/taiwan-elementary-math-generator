...
const htmlPath = path.join(OUTPUT, "g3b-u09-decimal-read-write-compose.html");
const pdfPath = path.join(OUTPUT, "g3b-u09-decimal-read-write-compose.pdf");
...
const semanticScopeFindingCount = evidence.generation.questions.filter((row) => {
  const canonicalInvalid = row.finalAnswer?.scale !== 1 || row.finalAnswer?.canonicalText !== row.decimalValue || row.finalAnswer?.exact !== true;
  const readWriteInvalid = row.kind === "g3bU09DecimalReadWrite" && (!row.decimalReading || !["notation_to_reading", "reading_to_notation"].includes(row.readWriteDirection));
  const composeInvalid = row.kind === "g3bU09DecimalComposeDecompose" && (row.placeUnit !== "0.1" || row.answerText !== `${row.whole}.${row.fractionalUnits}`);
  return row.questionMode !== "numeric" || row.metadata?.applicationClassification !== "APPLICATION_NOT_APPLICABLE" || canonicalInvalid || readWriteInvalid || composeInvalid || /(?:算式|_{2,}|答\s*[:：]|\{\{)/.test(row.blankedDisplayText ?? "");
}).length;
const report = {
...
if (report.questionCount !== 8 || report.answerKeyItemCount !== 8 || report.knowledgePointIds.length !== 2 || report.patternSpecIds.length !== 2 || report.patternAllocation.length !== 2 || report.patternAllocation.some((row) => row.questionCount !== 4) || report.canonicalDecimalIdentities.length !== 8 || report.duplicatePromptFindingCount !== 0 || report.physicalPdfPageCount !== report.questionPageCount + report.answerKeyPageCount || report.physicalPdfPageCount !== 2 || report.overflowFindingCount !== 0 || report.consoleErrorCount !== 0 || report.pageErrorCount !== 0 || report.semanticScopeFindingCount !== 0) throw new Error(`P03F8_CHROMIUM_ACCEPTANCE_FAILED:${JSON.stringify(report)}`);
