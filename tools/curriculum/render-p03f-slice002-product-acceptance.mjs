...
try {
  for (const mode of ["numeric", "application"]) {
    ...
    await page.pdf({ path: path.join(OUTPUT, `g3a-u08-fraction-quantity-${mode}.pdf`), format: "A4", printBackground: true, preferCSSPageSize: true, margin: { top: "0", right: "0", bottom: "0", left: "0" } });
    browserFindings[mode] = { pageMetrics, consoleErrors, pageErrors };
    await page.close();
  }
} finally {
  await browser.close();
...
const report = {
  ...
  browserFindings,
  visualReview: { status: "PENDING", numericQuestionPageReviewed: false, numericAnswerKeyPageReviewed: false, applicationQuestionPageReviewed: false, applicationAnswerKeyPageReviewed: false, clippedTextFindingCount: null, overlapFindingCount: null, brokenGlyphFindingCount: null, semanticScopeFindingCount: null },
};
fs.writeFileSync(path.join(OUTPUT, "p03f-slice002-product-acceptance-report.json"), JSON.stringify(report, null, 2) + "\n");
if (report.numericQuestionCount !== 6 || report.applicationQuestionCount !== 6 || report.answerKeyItemCount !== 12 || report.patternSpecIds.length !== 6 || report.contextMacroIds.length !== 3 || report.globalContextBindingCount !== 3 || report.overflowFindingCount !== 0 || report.consoleErrorCount !== 0 || report.pageErrorCount !== 0 || report.semanticScopeFindingCount !== 0) {
