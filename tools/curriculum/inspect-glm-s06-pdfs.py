#!/usr/bin/env python3
"""Inspect transient GLM-S06 PDFs for blank-page and text-boundary defects."""
...
def main() -> None:
    if len(sys.argv) != 3:
        raise SystemExit("usage: inspect-glm-s06-pdfs.py <input.json> <output.json>")
    ...
    if missing:
        raise SystemExit(f"GLM_S06_PDF_MISSING:{missing[:3]}")

    temporary_root = Path(tempfile.mkdtemp(prefix="glm-s06-pdf-inspect-"))
    try:
        results = {str(path): inspect_pdf(path, temporary_root) for path in pdf_paths}
    finally:
    ...
    output_path.write_text(
        json.dumps(
            {"schemaVersion": "glm-s06-pdf-inspection-v1", "results": results},
            ensure_ascii=False,
            indent=2,
        )
        + "\n",
