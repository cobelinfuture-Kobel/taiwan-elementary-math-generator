...
const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "../../docs/curriculum/output/experiments/g4b-u04-r3b-2x6");
const SOURCE_ID = "g4b_u04_4b04";
const QUESTION_COUNT = 200;
const EXPERIMENTAL_COLUMNS = 2;
const EXPERIMENTAL_ROWS = 6;
...
const options = {
  ...
  generationSeed: "g4b-u04-r3b-2x6-test-only",
...
if (!result.ok || !result.worksheetDocument) {
  throw new Error(`R3B 2x6 experiment generation failed: ${JSON.stringify(result.errors)}`);
}
if ((result.validation?.errors ?? []).length !== 0) {
  throw new Error(`R3B 2x6 experiment blocking validation errors: ${JSON.stringify(result.validation.errors)}`);
}
...
if (questionDisplayModels.length !== QUESTION_COUNT || signatures.length !== QUESTION_COUNT) {
  throw new Error(`R3B 2x6 experiment question count mismatch: ${questionDisplayModels.length}/${signatures.length}`);
}
if (new Set(signatures).size !== signatures.length) {
  throw new Error("R3B 2x6 experiment duplicate normalized prompt signature");
}
if (questionDisplayModels.some((model) => model.responsePrompt)) {
  throw new Error("R3B 2x6 experiment response prompt was not removed");
}
...
if (questionPages.length !== EXPECTED_PAGE_COUNT) {
  throw new Error(`R3B 2x6 experiment page count mismatch: ${questionPages.length}/${EXPECTED_PAGE_COUNT}`);
}
...
const experimentLayout = {
  ...
  includeAnswerKey: false,
  appliedLayoutText: "測試版面：題目 2 欄 × 6 列；只顯示題目文字",
};
...
const worksheetDocument = {
  ...
  layoutNoticeText: "此為只顯示題目文字的 2×6 測試產物，尚未套用到正式網站。",
  metadata: {
...
let html = renderWorksheetDocumentToHtml(worksheetDocument, {
  title: "4B-U04 概數｜2×6 題目-only 測試版｜200 題",
  stylesheetHref: "../../../../../site/assets/styles/print-styles.css",
  debugDataAttributes: false,
});
html = html
  .replace("<head>", '<head><meta name="robots" content="noindex,nofollow"><meta name="generator" content="G4B-U04 R3B 2x6 question-only test experiment">')
  .replace(
    '<body class="worksheet-renderer worksheet-renderer--g4b-u04"',
    '<body class="worksheet-renderer worksheet-renderer--g4b-u04" data-r3b-test-only="true" data-r3b-question-only="true" data-r3b-columns="2" data-r3b-rows="6"',
  );
const htmlContents = `${html}\n`;
if (/<div class="g4b-u04-cell__response"/.test(htmlContents)) {
  throw new Error("R3B 2x6 question-only HTML still contains response prompt markup");
}
const htmlFile = "g4b-u04-r3b-2x6-200q-question-only.html";
const pdfFile = "g4b-u04-r3b-2x6-200q-question-only.pdf";
writeFileSync(resolve(ROOT, htmlFile), htmlContents, "utf8");

const manifest = {
  schemaVersion: "g4b-u04-r3b-2x6-test-only-v1",
  task: "G4B_U04_R3B_2X6_QuestionOnlyTestExperiment",
