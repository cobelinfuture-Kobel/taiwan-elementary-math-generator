...
const FILES = [
  "g5a-u04-simplest-fraction.html",
  "g5a-u04-simplest-fraction.pdf",
  "p03f-slice013-product-acceptance-report.json",
];
...
if (manifest.status !== "SLICE013_RUNTIME_CONNECTED_PENDING_CHROMIUM_ACCEPTANCE") {
  throw new Error(`P03F13_MATERIALIZATION_STATE_INVALID:${manifest.status}`);
}
for (const name of FILES) {
  if (!fs.existsSync(path.join(TMP, name))) {
    throw new Error(`P03F13_MATERIALIZATION_INPUT_MISSING:${name}`);
  }
}
...
if (hashes.html !== report.htmlSha256 || hashes.pdf !== report.pdfSha256) {
  throw new Error("P03F13_MATERIALIZATION_HASH_COPY_MISMATCH");
}
const materializedReport = {
  ...
  materialization: {
    ...
    sourceHeadSha: process.env.P03F13_SOURCE_HEAD_SHA ?? null,
...
manifest.mainlineBoundary.slice013KnowledgePointAdmitted = false;
writeJson(MANIFEST_PATH, manifest);
console.log(`P03F13_RUNNER_ARTIFACT_MATERIALIZED=${JSON.stringify({
  hashes,
