...
const p=JSON.parse(readFileSync(new URL("../../data/curriculum/full-product/p06f/q012-g5b-u11-compare-construct-trend-source-authority-preflight.json",import.meta.url),"utf8"));
const slice=materializeP06EW6DirectProductVerticalSliceQueue().queueEntries[11];
const expected=["kp_g5b_u11_compare_two_data_series","kp_g5b_u11_construct_line_chart","kp_g5b_u11_line_chart_trend"];
...
if(JSON.stringify(slice.knowledgePointIds)!==JSON.stringify(expected))throw new Error("P06F_W6_Q012_KP_IDENTITY");
...
for(const kp of expected){const mapping=getR04KnowledgePointCapabilityMapping(kp);if(mapping?.primaryRuntimeProfileId!=="profile_chart_data")throw new Error(`P06F_W6_Q012_PROFILE:${kp}`);}
