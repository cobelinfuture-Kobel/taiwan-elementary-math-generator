...
test("Batch A selector state defaults to source-unit mode with current visible KP counts", () => {
  ...
  assert.equal(state.batchA.selectorAvailability.visibleCount, 29);
  assert.equal(state.batchA.selectorAvailability.bySourceId.g3a_u01_3a01.visibleCount, 1);
  ...
  assert.equal(state.batchA.selectorAvailability.bySourceId.g3b_u01_3b01.visibleCount, 5);
