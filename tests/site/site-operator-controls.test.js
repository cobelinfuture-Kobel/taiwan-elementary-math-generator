...
const G3A_U02_SUBTRACTION_REGROUP_POLICY = Object.freeze({ checkedColumns: ["ones", "tens", "hundreds"], minRegroupCount: 2 });
...
const G3A_U06_SPEC_IDS = new Set(["ps_g3a_u06_exact_division_check", "ps_g3a_u06_divisibility_exact_check", "ps_g3a_u06_division_with_remainder", "ps_g3a_u06_quotative_division_packaging", "ps_g3a_u06_partitive_division_equal_sharing", "ps_g3a_u06_parity_range_missing_digit"]);

function readText(relativePath) { return readFileSync(path.join(PROJECT_ROOT, relativePath), "utf8"); }
...
test("S43G2A0 - existing subtraction PatternSpec source-level smoke QA", () => {
  ...
  const subtractionQuestions = result.worksheetDocument.generatedQuestions.filter((question) => question.patternSpecId === G3A_U02_SUB_SPEC_ID);
...
test("Batch A controls - selecting one KP updates generated worksheet", () => {
  const state = createConfigState();
  setBatchASourceId(state, "g3a_u02_3a02");
  setBatchASelectorSelection(state, { selectionMode: BATCH_A_SELECTION_MODES.SINGLE_KNOWLEDGE_POINT, selectedKnowledgePointIds: [G3A_U02_ADD_KP_ID], selectedPatternGroupIds: [G3A_U02_ADD_GROUP_ID] });
  setBatchAQuestionCount(state, 6);
  const result = buildWorksheetDocumentFromState(state);
  assert.equal(result.ok, true, JSON.stringify(result.errors));
...
test("Batch A controls - selecting multiple same-unit KPs mixes patterns", () => {
  const state = createConfigState();
  setBatchASourceId(state, "g3a_u02_3a02");
  setBatchASelectorSelection(state, { selectionMode: BATCH_A_SELECTION_MODES.MIXED_KNOWLEDGE_POINTS_SAME_UNIT, selectedKnowledgePointIds: [G3A_U02_ADD_KP_ID, G3A_U02_SUB_KP_ID], selectedPatternGroupIds: [G3A_U02_ADD_GROUP_ID, G3A_U02_SUB_GROUP_ID] });
  setBatchAQuestionCount(state, 8);
  const result = buildWorksheetDocumentFromState(state);
  assert.equal(result.ok, true, JSON.stringify(result.errors));
...
test("legacy question count helper keeps allocation in sync", () => {
  ...
  assert.equal(state.draftConfig.patternPlan.allocation.totalQuestionCount, 12);
});
