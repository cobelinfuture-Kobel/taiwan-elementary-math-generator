...
test("PGC-R07 A01 binds the Classic failure to the latest deployed main replay", () => {
  assert.equal(deployed.status, "FAIL");
  assert.equal(deployed.run.id, "30534871170");
  assert.equal(Number(deployed.run.attempt), 2);
  ...
  assert.equal(evidence.runId, deployed.run.id);
  assert.equal(evidence.attempt, Number(deployed.run.attempt));
  assert.equal(evidence.deploymentSha, deployed.deploymentSha);
