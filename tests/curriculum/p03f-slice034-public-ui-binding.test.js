...
test("P03F34 pattern-group state remains intact while Pixel current registry advances through Slice052 to 34 sources / 258 KPs",()=>{
  ...
  const snapshot=getCurrentPixelRegistrySnapshot(); assert.equal(snapshot.sourceCount,34); assert.equal(snapshot.visibleKnowledgePointCount,258); assert.equal(snapshot.bySourceId[G4A_U09_P03F34_SOURCE_ID].visibleKnowledgePoints.length,7);
});
