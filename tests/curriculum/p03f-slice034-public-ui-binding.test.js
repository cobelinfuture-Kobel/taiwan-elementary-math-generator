...
test("P03F34 pattern-group state remains intact while Pixel current registry advances through Slice053 to 34 sources / 259 KPs",()=>{
  ...
  const snapshot=getCurrentPixelRegistrySnapshot(); assert.equal(snapshot.sourceCount, 35); assert.equal(snapshot.visibleKnowledgePointCount, 260); assert.equal(snapshot.bySourceId[G4A_U09_P03F34_SOURCE_ID].visibleKnowledgePoints.length,7);
});
