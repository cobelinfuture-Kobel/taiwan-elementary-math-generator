...
test("P03F34 pattern-group state and Pixel current registry reach 32 sources / 230 KPs",()=>{
  ...
  const snapshot=getCurrentPixelRegistrySnapshot(); assert.equal(snapshot.sourceCount,32); assert.equal(snapshot.visibleKnowledgePointCount,230); assert.equal(snapshot.bySourceId[G4A_U09_P03F34_SOURCE_ID].visibleKnowledgePoints.length,7);
});
