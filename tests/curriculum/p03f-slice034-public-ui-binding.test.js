...
test("P03F34 pattern-group state remains intact while Pixel current registry advances through reconciled Slice042 to 33 sources / 241 KPs",()=>{
  ...
  const snapshot=getCurrentPixelRegistrySnapshot(); assert.equal(snapshot.sourceCount,33); assert.equal(snapshot.visibleKnowledgePointCount,241); assert.equal(snapshot.bySourceId[G4A_U09_P03F34_SOURCE_ID].visibleKnowledgePoints.length,7);
});
