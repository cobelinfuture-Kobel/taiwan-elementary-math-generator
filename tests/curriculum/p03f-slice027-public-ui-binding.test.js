...
test("P03F27 current registry advances through Slice044 to exactly 33 public sources and 245 visible KPs", () => {
  const snapshot = getCurrentPixelRegistrySnapshot();
  assert.equal(snapshot.sourceCount, 33);
  assert.equal(snapshot.visibleKnowledgePointCount, 245);
