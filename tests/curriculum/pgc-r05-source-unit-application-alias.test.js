...
const W01_SOURCE_UNIT_ROUTE_IDS = Object.freeze([
...
const NON_W01_COMPATIBILITY_ROUTE_IDS = Object.freeze([
  "pgc_r03_g4a_u02_4a02_application_381f26d07d56",
  "pgc_r03_g4a_u04_4a04_application_371414272efb",
]);
...
function assertAcceptedApplicationRoute(route, routeId) {
...
test("PGC-R05 W01 source-unit routes retain application aliases and produce complete 20-question worksheets", () => {
  const report = loadReport();
  const byId = new Map(report.routes.map((route) => [route.routeId, route]));
  for (const routeId of W01_SOURCE_UNIT_ROUTE_IDS) assertAcceptedApplicationRoute(byId.get(routeId), routeId);
  assert.ok(report.summary.live20FailRouteCount <= 35, JSON.stringify(report.summary));
});

test("PGC-R05 non-W01 source-unit aliases preserve their admitted base-pattern compatibility", () => {
  const report = loadReport();
  const byId = new Map(report.routes.map((route) => [route.routeId, route]));
  for (const routeId of NON_W01_COMPATIBILITY_ROUTE_IDS) assertAcceptedApplicationRoute(byId.get(routeId), routeId);
});
...
test("PGC-R05 source-unit alias repair remains a shared projection fix within the frozen scope", () => {
  ...
  const targetIds = [...W01_SOURCE_UNIT_ROUTE_IDS, ...NON_W01_COMPATIBILITY_ROUTE_IDS];
  const repaired = report.routes.filter((route) => targetIds.includes(route.routeId));
  assert.equal(repaired.length, targetIds.length);
