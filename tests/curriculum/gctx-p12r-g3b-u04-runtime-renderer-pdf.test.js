...
import {
  ...
  G3B_U04_GLOBAL_CONTEXT_PILOT_WORKSHEET_QUESTION_COUNT
} from "../../site/modules/curriculum/batch-a/g3b-u04-global-context-pilot-worksheet-fullfix.js";
import {
  G3B_U04_GLOBAL_CONTEXT_PILOT_AUTHORITY_DOMAINS,
...
const EXPECTED_CONTEXT_PHRASES = ["班級園遊會", "戶外學習", "運動練習", "社區清潔活動", "露營活動"];

function targetQuestions(document) {
  return document.generatedQuestions.filter((question) => question.patternSpecId === G3B_U04_GLOBAL_CONTEXT_PILOT_PATTERN_SPEC_ID);
}

test("GCTX-P12R derives five target questions from the actual five-family visible resolver group", () => {
  ...
  assert.equal(G3B_U04_GLOBAL_CONTEXT_PILOT_RESOLVER_FAMILY_COUNT, 5);
  assert.equal(options.questionCount, G3B_U04_GLOBAL_CONTEXT_PILOT_WORKSHEET_QUESTION_COUNT);
  assert.equal(options.questionCount, 25);
  assert.equal(options.globalContextPilot.expectedTargetQuestionCount, 5);
...
test("GCTX-P12R builds baseline and pilot worksheets through the canonical pipeline", () => {
  ...
  assert.equal(result.baselineWorksheetDocument.generatedQuestions.length, 25);
  assert.equal(result.pilotWorksheetDocument.generatedQuestions.length, 25);
  assert.equal(result.targetQuestionIndexes.length, 5);
  assert.equal(result.variantIds.length, 5);
  assert.equal(result.resolverFamilyCount, 5);
  ...
  assert.equal(new Set(pilotTargets.map((question) => question.globalContextPilot.authorityContextDomain)).size >= 4, true);
  assert.equal(pilotTargets.every((question) => G3B_U04_GLOBAL_CONTEXT_PILOT_AUTHORITY_DOMAINS.includes(question.contextDomain)), true);
});
...
test("GCTX-P12R preserves canonical arithmetic, answer units, and answer-key parity", () => {
  ...
  for (const question of targetQuestions(document)) {
...
test("GCTX-P12R uses the existing S57F5 production renderer for before and after", () => {
  const result = buildG3BU04GlobalContextPilotWorksheet();
  assert.equal(result.ok, true, JSON.stringify(result.errors));
  const baselineHtml = renderWorksheetDocumentToHtml(result.baselineWorksheetDocument, { title: "GCTX-P12R Before", stylesheetHref: "", debugDataAttributes: false });
  const pilotHtml = renderWorksheetDocumentToHtml(result.pilotWorksheetDocument, { title: "GCTX-P12R After", stylesheetHref: "", debugDataAttributes: false });
...
test("GCTX-P12R blocks unregistered variants and public selection", () => {
  const unregistered = validateG3BU04GlobalContextPilotOptions({ globalContextPilot: { enabled: true, variantIds: ["gctx_unknown"] } });
  ...
  const publicSelection = validateG3BU04GlobalContextPilotOptions({
    globalContextPilot: { enabled: true, publicSelectable: true, variantIds: createG3BU04GlobalContextPilotOptions().globalContextPilot.variantIds }
  });
...
test("GCTX-P12R validator blocks prompt, arithmetic, and authority-domain mutations", () => {
  const result = buildG3BU04GlobalContextPilotWorksheet();
  assert.equal(result.ok, true, JSON.stringify(result.errors));
  ...
  question.globalContextPilot.authorityContextDomain = "wrong_domain";
  ...
  assert.ok(validation.errors.some((entry) => entry.code === "GCTX_P12R_AUTHORITY_DOMAIN_INVALID"));
});
