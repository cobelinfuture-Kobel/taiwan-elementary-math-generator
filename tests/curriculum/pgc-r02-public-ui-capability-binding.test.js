...
test("PGC-R02 R05 keeps Classic, 404 fallback and Pixel capacity parity", () => {
  ...
  for (const source of CURRENT_FULL_PRODUCT_PUBLIC_SOURCE_UNITS) {
    ...
    for (const input of cases) {
      const snapshots = Object.values(PUBLIC_UI_SURFACES).map((surfaceId)=>resolvePublicUiCapabilityBinding({ sourceId:source.sourceId, surfaceId, ...input }));
      const expected = { questionTypes:optionValues(snapshots[0]), max:snapshots[0].questionCount.max, status:snapshots[0].capacityStatus };
...
test("PGC-R02 R05 browser selector form registry contains every capacity-exposed form", () => {
  ...
  for (const kp of listVisibleBatchAKnowledgePoints()) {
    ...
    const expectedGroupIds = uniqueSorted(base.availableQuestionTypeOptions
      ...
      .flatMap((option)=>resolvePublicUiCapabilityBinding({ sourceId:kp.sourceId, selectionMode:"singleKnowledgePoint", selectedKnowledgePointIds:[kp.knowledgePointId], requestedQuestionType:option.value }).compatiblePatternGroupIds));
    const selectorGroupIds = new Set(listPublicPatternGroupChoices([kp.knowledgePointId]).map((choice)=>choice.patternGroupId));
    for (const patternGroupId of expectedGroupIds) assert.equal(selectorGroupIds.has(patternGroupId), true, `${kp.knowledgePointId}:${patternGroupId}`);
...
test("PGC-R02 R05 derives types from KP capability before filtering legal forms", () => {
  ...
  const withFormSelection = resolvePublicUiCapabilityBinding({ sourceId:witness.kp.sourceId, selectionMode:"singleKnowledgePoint", selectedKnowledgePointIds:[witness.kp.knowledgePointId], selectedPatternGroupIds:[selectedGroupId] });
