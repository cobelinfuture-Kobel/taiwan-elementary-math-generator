...
test("P03F40 current Pixel authority preserves the q040 admission floor while allowing q050 current promotion", () => {
  const pixel = getCurrentPixelRegistrySnapshot();
  assert.ok(pixel.sourceCount >= 33);
  assert.ok(pixel.visibleKnowledgePointCount >= 238);
  assert.equal(pixel.bySourceId[G5B_U06_P03F40_SOURCE_ID].visibleKnowledgePoints.length, 5);
  assert.equal(pixel.bySourceId[G5B_U06_P03F40_SOURCE_ID].hiddenPendingCount, 0);
  assert.equal(pixel.bySourceId[G5B_U06_P03F40_SOURCE_ID].notSelectableCount, 0);
});
