...
test("PGC-R05 G3B-U04 shared canonical retry eliminates all six prompt-collision routes", () => {
  ...
  assert.ok(report.summary.live20PassRouteCount >= 198, JSON.stringify(report.summary));
  assert.ok(report.summary.live20FailRouteCount <= 13, JSON.stringify(report.summary));
});
