...
test('M00 validates the exact 79-node scope with W01 admitted and W02 hidden PatternSpecs materialized', () => {
  ...
  assert.equal(result.status, 'W02_HIDDEN_PATTERNSPECS_MATERIALIZED');
  ...
  assert.equal(result.nextShortestStep, 'POSTG-APP-W02-A02_AtomicContextBindingAndSingleApplicationCandidateMaterialization');
});
...
test('Wave 02 has page-evidenced KP classification, canonical models and hidden PatternSpecs complete', () => {
  ...
  assert.equal(w02.currentState.state, 'HIDDEN_PATTERNSPECS_MATERIALIZED');
...
test('W01 stays E5 while W02 A00 through A01D remain E3 non-production', () => {
  ...
  assert.equal(controller.w02A01DClaim.actualEvidenceLevel, 'E3_SHADOW_RUNTIME_INTEGRATED');
  assert.equal(controller.w02A01DClaim.claims.productionAdmitted, false);
  assert.equal(controller.w02A01DClaim.claims.d0Complete, false);
...
test('forged approval, W02 claims and PDF evidence state fail closed', () => {
  ...
  const a01dClaimCase = structuredClone(controller);
  a01dClaimCase.w02A01DClaim.claims.productionAdmitted = true;
  assert.equal(codes(validatePOSTGAPPMasterController(a01dClaimCase)).includes('POSTG_APP_W02_A01D_CLAIM_INVALID'), true);
