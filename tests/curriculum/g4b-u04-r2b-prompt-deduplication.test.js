...
test("R2B capacity-aware allocation caps fixed prompt pools and redistributes mixed worksheets", () => {
  const ids = [
  ...
  assert.equal(result.patternAllocation.ps_g4b_u04_inverse_digit_set, 4);
  assert.equal(result.patternAllocation.ps_g4b_u04_inverse_original_values, 4);
  assert.equal(result.patternAllocation.ps_g4b_u04_round_half_up, 31);
});
...
test("R2B mixed 40-question canonical output contains no duplicate normalized prompts", () => {
  ...
  assert.equal(normalized.patternAllocation.ps_g4b_u04_inverse_digit_set, 4);
  assert.equal(normalized.patternAllocation.ps_g4b_u04_inverse_original_values, 4);

  const result = generateG4BU04CanonicalQuestions(normalized);
  assert.equal(result.ok, true, JSON.stringify(result.errors));
  assert.equal(result.questions.length, 40);
  const signatures = result.questions.map((row) => normalizeG4BU04PromptSignature(row.promptText));
