...
const ADMITTED_SOURCE_IDS = Object.freeze([
  "g5b_u05_5b05a",
  "g6a_u01_6a01",
  "g5a_u03_5a03a",
  "g5a_u03_5a03a1",
...
test("P01A accounts for all twenty-one admitted patterns and zero remaining vertical slices", () => {
  const inventory = materializeP01AW1ProductAdmissionInventory();
  ...
  assert.equal(inventory.rows.every((row) => row.productGapState === "ADMISSION_READY_EXISTING_PUBLIC_PATTERN"), true);
  assert.equal(inventory.rows.every((row) => row.currentProductCoverage.publicSourceSelectable === true), true);
  assert.equal(inventory.rows.every((row) => row.currentProductCoverage.patternSpecIds.length === 2), true);
  console.log(`P01A_REBASED_INVENTORY_READBACK=${JSON.stringify({ metrics: inventory.metrics, sourceSummaries: inventory.sourceSummaries })}`);
});

test("P01A preserves protected W0 while admitting four isolated W1 full-product sources", () => {
  const inventory = materializeP01AW1ProductAdmissionInventory();
  ...
  const admittedRows = inventory.rows.filter((row) => row.sourceNodeIds.some((id) => ADMITTED_SOURCE_IDS.includes(id)));
  assert.equal(admittedRows.length, 21);
  assert.equal(admittedRows.every((row) => row.currentProductCoverage.publicSourceSelectable === true), true);
});

test("P01A validator passes the post-P01D3 executable gap matrix", () => {
  ...
  assert.equal(report.summary.admissionReadyExistingPublicPatternCount, 21);
  assert.equal(report.summary.publicProductVerticalSliceRequiredCount, 0);
...
test("P01A validator fails closed if inventory is misreported as directly admitted", () => {
  ...
  const tampered = { ...inventory, rows, metrics: { ...inventory.metrics, directProductionAdmissionCount: 1 } };
...
test("P01A keeps public UI closeout and recursive-improvement administration behind their gates", () => {
  const inventory = materializeP01AW1ProductAdmissionInventory();
  assert.equal(inventory.policy.inventoryDecision.selectedNextTask, "P01E_W1PublicUIHTMLPDFPrintCloseout");
