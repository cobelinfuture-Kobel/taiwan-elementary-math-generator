...
test("S91 audit covers 22 routes, 14 Class C, 8 Class D, and 16 answer models", () => {
  ...
  assert.equal(audit.supportedAnswerModelCount, 16);
  assert.equal(patternIds.length, 22);
  assert.equal(getG5AU02HiddenWorksheetAnswerModelIds().length, 16);
});

test("S91 normalizes and allocates an exact canonical round-robin count", () => {
  const selected = patternIds.slice(0, 3);
  const allocation = allocateG5AU02HiddenWorksheet({
  ...
  assert.deepEqual(allocation.allocation.patternSequence.slice(0, 6), [
    selected[0], selected[1], selected[2], selected[0], selected[1], selected[2],
  ]);
  assert.deepEqual(allocation.allocation.patternCounts, {
    [selected[0]]: 17,
    [selected[1]]: 17,
    [selected[2]]: 16,
  });
...
test("S91 builds one hidden question and answer for every canonical PatternSpec", () => {
  const result = buildG5AU02HiddenWorksheetDocument({
    questionCount: 22,
    baseSeed: 20260714,
  ...
  assert.equal(new Set(document.questionRecords.map((row) => row.answerModelId)).size, 16);
...
test("S91 supports exact counts above route count and is deterministic by seed", () => {
  const options = {
    patternSpecIds: patternIds.slice(0, 5),
    questionCount: 47,
    baseSeed: 991,
    questionRowsPerPage: 9,
    answerRowsPerPage: 10,
  };
...
test("S91 suppresses all answer records and pages when the answer key is disabled", () => {
  const result = buildG5AU02HiddenWorksheetDocument({
    patternSpecIds: patternIds.slice(8, 13),
    questionCount: 13,
    baseSeed: 319,
