...
test("selector candidate projection exposes the first visible G3A-U02 knowledge point", () => {
  assert.deepEqual(listVisibleBatchAKnowledgePoints().map((row) => row.knowledgePointId), [VISIBLE_KP_ID]);
...
test("add multi-carry visible selector candidate resolves to its PatternGroup and PatternSpec", () => {
  const visibleKnowledgePoint = getVisibleBatchAKnowledgePoint(VISIBLE_KP_ID);

  assert.equal(visibleKnowledgePoint.knowledgePointId, VISIBLE_KP_ID);
  assert.deepEqual(visibleKnowledgePoint.patternGroupIds, [VISIBLE_PATTERN_GROUP_ID]);
  assert.deepEqual(visibleKnowledgePoint.patternSpecIds, [VISIBLE_PATTERN_SPEC_ID]);
  assert.equal(visibleKnowledgePoint.qaStatusLabel, "qa_verified");

  assert.deepEqual(getVisiblePatternGroupsForKnowledgePoint(VISIBLE_KP_ID).map((row) => row.patternGroupId), [VISIBLE_PATTERN_GROUP_ID]);
  assert.deepEqual(resolveVisiblePatternSpecIdsForKnowledgePoint(VISIBLE_KP_ID), [VISIBLE_PATTERN_SPEC_ID]);
});

test("hidden subtract A-row and D rows are not visible selector candidates", () => {
