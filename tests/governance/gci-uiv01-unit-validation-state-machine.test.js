...
test("Layer 2 files explicitly exclude GitHub CI enforcement ownership", () => {
  ...
  assert.equal(POLICY.layerStatus.layer3GitHubCiEnforcement, "NOT_STARTED");
...
test("Unit mainline advances one KP at a time and cannot skip completeness", () => {
  ...
  assert.equal(r.state, "KP_VALIDATING");
  assert.equal(r.lastDerivedLane, "KP_FOCUSED");
  ...
  assert.equal(r.state, "KP_FOCUSED_PASS");
  assert.equal(r.knowledgePointGateStatus.kp_a, "FOCUSED_PASS");
...
test("Unit integration is legal only after all expected KPs focused-pass and derives one full-regression lane", () => {
  const complete = record({
    state: "UNIT_KPS_COMPLETE",
    knowledgePointGateStatus: { kp_a: "FOCUSED_PASS", kp_b: "FOCUSED_PASS" },
  });
  const r = transitionUnitValidationState({
    ...
    request: {
      ...
      changeImpact: impact({ publicAuthorityCutover: true, currentAuthorityChanged: true }),
    },
  });
  assert.equal(r.state, "UNIT_INTEGRATING");
...
test("Same exact-head full regression PASS must be reused", () => {
  ...
  assert.equal(decision.evidenceId, "full-pass-a");
  ...
  assert.equal(changedHead.runFullRegression, true);
});

test("Bounded shared runtime uses SHARED_BOUNDED; unbounded or legal-route changes use GLOBAL_CERTIFICATION", () => {
  assert.equal(
...
test("Unit integration PASS consumes required exact-head evidence and closeout cannot trigger a new regression", () => {
  let r = record({
    ...
    knowledgePointGateStatus: { kp_a: "FOCUSED_PASS", kp_b: "FOCUSED_PASS" },
    lastDerivedLane: "UNIT_FULL_ONCE",
  });
  const rows = [
  ...
  assert.equal(r.state, "UNIT_VALIDATED");
...
test("Illegal state jumps and evidence bound to the wrong head fail closed", () => {
  assert.throws(
