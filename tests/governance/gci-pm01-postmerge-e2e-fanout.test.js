...
test("GCI-PM01 policy is tied to Q010 D0 evidence and exact observed fanout",()=>{
  ...
  assert.equal(policy.historicalCutoverWorkflows.length,46);
  assert.equal(new Set(policy.historicalCutoverWorkflows).size,46);
});
...
test("Q019 is the single current shared-path owner and Q018 is demoted to owned paths only",()=>{
  ...
  assert.match(text,/name: P07F W7 Q019 Post-Merge Pages E2E/);
  assert.match(text,/node tools\/curriculum\/run-p07f-w7-q019-live-pages-e2e\.mjs/);
  assert.ok(policy.historicalCutoverWorkflows.includes(".github/workflows/p07f-w7-q018-live-pages-e2e.yml"));
