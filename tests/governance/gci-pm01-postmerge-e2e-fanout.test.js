...
test("GCI-PM01 policy is tied to Q010 D0 evidence and exact observed fanout",()=>{
  ...
  assert.equal(policy.historicalCutoverWorkflows.length,37);
...
test("Q010 remains the single current shared-path owner without semantic workflow mutation",()=>{
  ...
  assert.match(text,/name: P07F W7 Q010 Post-Merge Pages E2E/);
  assert.match(text,/node tools\/curriculum\/run-p07f-w7-q010-live-pages-e2e\.mjs/);
