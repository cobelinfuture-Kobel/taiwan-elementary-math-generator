...
test("GCI-PM01 policy is tied to Q010 D0 evidence and exact observed fanout",()=>{
  ...
  assert.equal(policy.historicalCutoverWorkflows.length,48);
  assert.equal(new Set(policy.historicalCutoverWorkflows).size,48);
});
...
test("Q021 is the single current shared-path owner and Q020 is demoted to owned paths only",()=>{
  ...
  assert.match(text,/name: P07F W7 Q021 Post-Merge Pages E2E/);
  assert.match(text,/node tools\/curriculum\/run-p07f-w7-q021-live-pages-e2e\.mjs/);
  assert.ok(policy.historicalCutoverWorkflows.includes(".github/workflows/p07f-w7-q020-live-pages-e2e.yml"));
