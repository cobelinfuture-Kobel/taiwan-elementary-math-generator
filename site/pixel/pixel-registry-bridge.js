...
export function listPixelSourceOptions() {
  return currentPixelSurfaceActive() ? listCurrentPixelSourceOptions() : mapPixelSourceOptions(listFullProductPublicSourceUnits());
}
...
export function listPixelKnowledgePointsForSource(sourceId) {
  return listVisibleBatchAKnowledgePoints().filter((entry) => entry.sourceId === sourceId).map((entry) => Object.freeze({
...
export function getPixelRegistrySnapshot() {
  ...
  return registrySnapshot(sources, current ? CURRENT_SELECTOR_AVAILABILITY.visibleCount : P01E_SELECTOR_AVAILABILITY.visibleCount);
}
