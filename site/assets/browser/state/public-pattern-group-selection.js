...
} from "../../../modules/curriculum/registry/batch-a-selector-p03f22-extension.js";
...
export function listPublicPatternGroupChoices(selectedKnowledgePointIds = []) {
  ...
  for (const knowledgePointId of uniqueStrings(selectedKnowledgePointIds)) {
    ...
    for (const group of groups) {
      choices.push({ ...clone(group), knowledgePointId, knowledgePointDisplayName: knowledgePoint.displayName, displayLabel: getPublicPatternGroupDisplayLabel(group), hasRepresentationChoice: groups.length > 1 });
    }
  }
  return choices;
}
...
export function normalizePublicPatternGroupSelection({ selectionMode = SOURCE_UNIT_MODE, selectedKnowledgePointIds = [], selectedPatternGroupIds = [] } = {}) {
  ...
  if (droppedIds.length > 0) warnings.push({ code: PUBLIC_PATTERN_GROUP_WARNING_CODES.PATTERN_GROUP_DROPPED, count: droppedIds.length });
  for (const knowledgePointId of knowledgePointIds) {
    ...
    const requestedForKnowledgePoint = groups.filter((group) => requestedIds.includes(group.patternGroupId)).map((group) => group.patternGroupId);
    ...
    if (defaultGroupId) {
      ...
      if (requestedIds.length > 0) warnings.push({ code: PUBLIC_PATTERN_GROUP_WARNING_CODES.PATTERN_GROUP_DEFAULTED, knowledgePointId });
    }
  }
