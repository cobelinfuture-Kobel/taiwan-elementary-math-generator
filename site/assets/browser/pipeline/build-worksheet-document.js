...
function resolveCloseoutApplicationPlan(publicPlan = {}) {
  ...
  const groups = knowledgePoints.flatMap((knowledgePoint) => [
    ...
    ...listW1FullProductPublicApplicationGroupsForKnowledgePoint(knowledgePoint.knowledgePointId),
  ].filter(groupLooksApplication));
  const uniqueGroups = [...new Map(groups.map((group) => [group.patternGroupId, group])).values()];
  const selectedKnowledgePointIds = [...new Set(uniqueGroups.flatMap((group) => [group.primaryKnowledgePointId, ...(group.knowledgePointIds ?? [])].filter(Boolean)))];
...
function blockedAuthoritativeCutoverResult(cutover, publicPlan) {
  return Object.freeze({ ok: false, errors: cutover.errors, warnings: Object.freeze([]), worksheetDocument: null, authoritativeConsumerCutover: cutover, requestedPlan: publicPlan });
}
function attachAuthoritativeCutover(result, cutover) {
  ...
  const projectedDocument = worksheetDocument ? {
    ...worksheetDocument,
    metadata: { ...(worksheetDocument.metadata ?? {}), r07AuthoritativeConsumerCutover: cutover.adapter },
    configSnapshot: { ...(worksheetDocument.configSnapshot ?? {}), globalAuthorityCutover: cutover.adapter },
    publicControls: { ...(worksheetDocument.publicControls ?? {}), authorityMode: "GLOBAL_PRIMARY" },
  } : worksheetDocument;
  return Object.freeze({ ...result, worksheetDocument: projectedDocument, authoritativeConsumerCutover: cutover });
}
...
export function buildWorksheetDocumentFromPlan(publicPlan) {
  ...
  const result = plan?.questionMode === "pbl" ? buildFifteenUnitPublicPblWorksheetResult(plan) : buildCoreWorksheetDocumentFromPlan(plan);
