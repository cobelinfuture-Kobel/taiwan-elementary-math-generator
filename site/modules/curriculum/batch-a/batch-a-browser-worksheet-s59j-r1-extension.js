...
import { buildBatchABrowserWorksheetDocument as buildBaseBatchABrowserWorksheetDocument } from "./batch-a-browser-worksheet-p03f20-extension.js";
...
function localizeWarning(issue = {}) {
  ...
  return { ...issue, severity: "warning", message: PUBLIC_WARNING_MESSAGE_BY_CODE[code] ?? "題目已通過驗證；非阻塞提示已自動彙整。" };
}
...
function isG4BU01Result(result = {}) { return result?.worksheetDocument?.batchA?.sourceId === G4B_U01_PUBLIC_WARNING_FULLFIX.sourceId; }
export function buildBatchABrowserWorksheetDocument(options = {}) {
  const result = buildBaseBatchABrowserWorksheetDocument(options);
  if (!isG4BU01Result(result)) return result;
  const warnings = dedupeG4BU01PublicIssues([...(result.warnings ?? []), ...(result.validation?.warnings ?? [])]);
  const validation = result.validation ? { ...result.validation, warnings } : result.validation;
  const worksheetDocument = {
    ...result.worksheetDocument,
    validationSummary: result.worksheetDocument?.validationSummary ? { ...result.worksheetDocument.validationSummary, warnings } : result.worksheetDocument?.validationSummary,
    generationReport: result.worksheetDocument?.generationReport
      ? { ...result.worksheetDocument.generationReport, validationWarnings: warnings, generationWarnings: warnings }
      : result.worksheetDocument?.generationReport,
  };
  return { ...result, worksheetDocument, validation, warnings };
}
