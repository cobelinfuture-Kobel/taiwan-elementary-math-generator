...
import { buildBatchABrowserPlan, requestsP03F8DecimalComposeDecompose } from "./batch-a-browser-generator-p03f8.js";
...
export const P03F8_WORKSHEET_ADAPTER = Object.freeze({
  task: "P03F_W3DirectProductVerticalSlice008Implementation",
...
function model(question, index, showNumber) {
  ...
  return Object.freeze({
    ...
    layoutHints: Object.freeze({ estimatedTextLength: String(prompt).length, hasGrouping: false, avoidPageBreakInside: true, representation: "decimal_compose_decompose_numeric", longTextCardPolicy: "avoidSplit" }),
  });
}

export function buildBatchABrowserWorksheetDocument(options = {}) {
  if (!requestsP03F8DecimalComposeDecompose(options)) return buildBase(options);
  ...
  const answers = printLayout.showAnswerKeyPage
    ? generation.questions.map((question, index) => Object.freeze({
      ...
      layoutHints: Object.freeze({ avoidPageBreakInside: true, representation: "decimal_compose_decompose_answer" }),
    }))
    : [];
  ...
  const document = Object.freeze({
    ...
    version: "1",
    worksheetId: `p03f8-numeric-${plan.questionCount}-${plan.generationSeed}`,
    worksheetKind: "batchAWorksheet",
    title: options.title ?? "三年級｜個位與十分位組成分解｜數字題",
    subtitle: "依個位與十分位組成一位小數",
    ...
    printOptions: Object.freeze({ ...printLayout, answerKeyColumns: 2, answerKeyRowsPerPage: 4, showAnswerKey: printLayout.showAnswerKeyPage, answerKeyPlacement: printLayout.showAnswerKeyPage ? "afterQuestions" : "none" }),
    publicControls: Object.freeze({ sourceId: G3B_U09_SOURCE_ID, questionMode: "numeric", productAdmissionTask: P03F8_WORKSHEET_ADAPTER.task, globalContextRegistry: null }),
    metadata: Object.freeze({ sourceId: G3B_U09_SOURCE_ID, knowledgePointId: G3B_U09_DECIMAL_COMPOSE_DECOMPOSE_KP_ID, applicationClassification: "APPLICATION_NOT_APPLICABLE", worksheetAdapter: P03F8_WORKSHEET_ADAPTER }),
