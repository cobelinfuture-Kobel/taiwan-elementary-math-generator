...
import {generateBatchABrowserQuestions,requestsP07F02} from "./batch-a-browser-generator-p07f02.js";
import {buildBatchABrowserWorksheetDocument as baseWorksheet} from "./batch-a-browser-worksheet-p07f01-extension.js";
import {validateG6AU05P07F02Question} from "./g6a-u05-ratio-value-runtime-p07f02.js";
import {G6A_U05_P07F02_KP_ID as KP,G6A_U05_P07F02_SOURCE_ID as SRC} from "../registry/g6a-u05-ratio-value-selector-projection-p07f02.js";
...
export function buildBatchABrowserWorksheetDocument(o={}){
  if(!requestsP07F02(o))return baseWorksheet(o);
  const generation=generateBatchABrowserQuestions(o);
  if(!generation?.ok)return Object.freeze({ok:false,errors:generation?.errors??["P07F02_GENERATION_FAILED"],warnings:generation?.warnings??[],worksheetDocument:null,generation});
  const ve=generation.questions.flatMap(q=>validateG6AU05P07F02Question(q).errors);
  ...
  if(generation.questions.some(q=>q.knowledgePointId!==KP))return Object.freeze({ok:false,errors:Object.freeze(["P07F02_WORKSHEET_KP_INVALID"]),warnings:Object.freeze([]),worksheetDocument:null,generation});
  ...
  const worksheetDocument=Object.freeze({
    worksheetKind:"batch_a",worksheetId:"p07f02-"+SRC+"-"+(o.generationSeed??"public"),title:"比和比值｜比值",
    ...
    metadata:Object.freeze({
      taskId:"P07F_W7DirectProductVerticalSlice002Implementation",sourceId:SRC,knowledgePointId:KP,questionMode:"numeric",
      ...
      applicationContextUsed:false,sameUnitMixedUsed:false,crossUnitMixedUsed:false,q003OrLaterTouched:false,sharedRuntimeScope:"SHARED_RUNTIME_BOUNDED",frozenRuntimeProfile:"profile_ratio_percent"
    }),
    summary:Object.freeze({questionCount:generation.questions.length,questionPageCount:questionPages.length,answerKeyPageCount:answerKeyPages.length,ratioValueQuestionCount:generation.questions.length,applicationQuestionCount:0})
  });
  return Object.freeze({ok:true,errors:Object.freeze([]),warnings:Object.freeze([]),worksheetDocument,generation,p07f02Implemented:true,q001ProductMutation:false,q003OrLaterTouched:false});
}
