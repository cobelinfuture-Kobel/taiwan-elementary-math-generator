...
import {generateBatchABrowserQuestions,requestsP07F04} from "./batch-a-browser-generator-p07f04.js";
import {buildBatchABrowserWorksheetDocument as baseWorksheet} from "./batch-a-browser-worksheet-p07f03-extension.js";
import {validateG6AU06P07F04Question} from "./g6a-u06-pi-circumference-relation-runtime-p07f04.js";
import {G6A_U06_P07F04_KP_ID as KP,G6A_U06_P07F04_SOURCE_ID as SRC} from "../registry/g6a-u06-pi-circumference-relation-selector-projection-p07f04.js";
...
export function buildBatchABrowserWorksheetDocument(o={}){
  if(!requestsP07F04(o))return baseWorksheet(o);
  const generation=generateBatchABrowserQuestions(o);
  if(!generation?.ok)return Object.freeze({ok:false,errors:generation?.errors??["P07F04_GENERATION_FAILED"],warnings:generation?.warnings??[],worksheetDocument:null,generation});
  const ve=generation.questions.flatMap(q=>validateG6AU06P07F04Question(q).errors);
  ...
  if(generation.questions.some(q=>q.knowledgePointId!==KP))return Object.freeze({ok:false,errors:Object.freeze(["P07F04_WORKSHEET_KP_INVALID"]),warnings:Object.freeze([]),worksheetDocument:null,generation});
  ...
  const worksheetDocument=Object.freeze({
    worksheetKind:"batch_a",worksheetId:"p07f04-"+SRC+"-"+(o.generationSeed??"public"),title:"圓周長與扇形周長｜圓周率與圓周長關係",
    ...
    metadata:Object.freeze({taskId:"P07F_W7DirectProductVerticalSlice004Implementation",sourceId:SRC,knowledgePointId:KP,questionMode:"diagram",sourceBackedDiagram:true,frozenRuntimeProfile:"profile_geometry_formula",circumferenceDiameterRelationOwned:true,circumferenceFormulaTeachingReowned:false,semicirclePerimeterReowned:false,sectorArcLengthReowned:false,compositeArcPerimeterReowned:false,rollingWheelApplicationUsed:false,genericGeometryFormulaDrillUsed:false,applicationContextUsed:false,sameUnitMixedUsed:false,crossUnitMixedUsed:false,q005OrLaterTouched:false,sharedRuntimeScope:"SHARED_RUNTIME_BOUNDED"}),
  ...
  return Object.freeze({ok:true,errors:Object.freeze([]),warnings:Object.freeze([]),worksheetDocument,generation,p07f04Implemented:true,q003ProductMutation:false,q005OrLaterTouched:false});
}
