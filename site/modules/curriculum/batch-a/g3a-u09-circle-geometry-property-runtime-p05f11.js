...
function diagramFor(spec,v){
  ...
  if(spec.knowledgePointId===G3A_U09_P05F11_KP_IDS[1]){
    if(spec.relation.startsWith("CLASSIFY_TWO_CIRCLE")){
      ...
      const secondRadiusCm=2+((v*3)%8),tangentMode=relationClass==="TANGENCY"?(v%2===0?"EXTERNAL":"INTERNAL"):null;
      return Object.freeze({...base,diagramMode:"TWO_CIRCLE_RELATION",relationClass,secondRadiusCm,secondRadiusPx:28+((v*7)%17),tangentMode});
    }
...
function promptFor(spec,d){
  switch(spec.relation){
    ...
    case "MEASURE_CIRCLE_RADIUS_FROM_DIAGRAM":return `圖中標示的半徑長 ${d.radiusCm} 公分，圓的半徑是多少公分？`;
    case "MEASURE_CIRCLE_DIAMETER_FROM_DIAGRAM":return `圖中通過圓心的直徑長 ${d.diameterCm} 公分，圓的直徑是多少公分？`;
...
function signatureFor(question){const d=question.geometryDiagram;return [question.patternSpecId,d.variant,d.radiusCm,d.radiusPx,d.rotationDeg,d.diagramMode,d.positionClass??"",d.distanceCm??"",d.relationClass??"",d.secondRadiusCm??"",d.tangentMode??"",d.segmentMode??"",String(d.isDiameter)].join("|");}
