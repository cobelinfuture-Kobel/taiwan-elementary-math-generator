...
import {generateBatchABrowserQuestions} from "./batch-a-browser-generator-p05f2.js";
import {validateG3AU09P05F2Question} from "./g3a-u09-circle-parts-runtime-p05f2.js";
import {G3A_U09_P05F2_KP_ID,G3A_U09_P05F2_SOURCE_ID} from "../registry/g3a-u09-circle-parts-selector-projection-p05f2.js";
...
function displayModels(questions,layout) {
  return questions.map((question,index)=>Object.freeze({
    ...
    layoutHints:Object.freeze({estimatedTextLength:question.blankedDisplayText.length,hasGrouping:false,avoidPageBreakInside:true,questionMode:"diagram",representation:"circle_parts_diagram"}),
  }));
}
function answerItems(questions,models) {
  return questions.map((question,index)=>Object.freeze({
    ...
    layoutHints:Object.freeze({avoidPageBreakInside:true,questionMode:"diagram",representation:"circle_parts_diagram"}),
  }));
}

export function buildBatchABrowserWorksheetDocument(options={}) {
  const generation=generateBatchABrowserQuestions(options);
  if (!generation?.ok) return Object.freeze({ok:false,errors:generation?.errors??["P05F2_GENERATION_FAILED"],warnings:generation?.warnings??[],worksheetDocument:null,generation});
  const validationErrors=generation.questions.flatMap((question)=>validateG3AU09P05F2Question(question).errors);
  ...
  const worksheetDocument=Object.freeze({
    ...
    worksheetId:`p05f2-${G3A_U09_P05F2_SOURCE_ID}-${options.generationSeed??"public"}`,
    title:"圓",
    ...
    publicControls:Object.freeze({sourceId:G3A_U09_P05F2_SOURCE_ID,questionMode:"diagram",questionCountMax:240}),
    configSnapshot:Object.freeze({questionMode:"diagram",printLayout:layout}),
    batchA:Object.freeze({sourceId:G3A_U09_P05F2_SOURCE_ID,questionMode:"diagram",selectionMode:options.selectionMode??"sourceUnit"}),
    metadata:Object.freeze({taskId:"P05F_W5DirectProductVerticalSlice002Implementation",sourceId:G3A_U09_P05F2_SOURCE_ID,knowledgePointId:G3A_U09_P05F2_KP_ID,questionMode:"diagram",sourceBackedDiagram:true,sharedRuntimeScope:"SHARED_RUNTIME_BOUNDED",q001SemanticsTouched:false,applicationContextUsed:false}),
  ...
  return Object.freeze({ok:true,errors:Object.freeze([]),warnings:Object.freeze([]),worksheetDocument,generation,p05f2Implemented:true,q001SemanticsTouched:false});
}
