...
import { buildBatchABrowserWorksheetDocument as buildBase } from "./batch-a-browser-worksheet-p03f14-extension.js";
import { buildBatchABrowserPlan, requestsP03F15 } from "./batch-a-browser-generator-p03f15.js";
import { generateBatchABrowserQuestions } from "./batch-a-browser-question-router-p03f15.js";
import { validateBatchABrowserPlan, validateBatchABrowserQuestions } from "./batch-a-browser-validator-p03f15.js";
import { G3B_U07_SAME_DENOMINATOR_SOURCE_ID } from "../registry/g3b-u07-same-denominator-selector-projection.js";

export const P03F15_WORKSHEET_ADAPTER = Object.freeze({
  task: "P03F_W3DirectProductVerticalSlice015Implementation",
  status: "bounded_shared_same_denominator_fraction_worksheet_adapter_connected",
...
export function buildBatchABrowserWorksheetDocument(options = {}) {
  if (!requestsP03F15(options)) return buildBase(options);
  ...
  const models = generation.questions.map((question, index) => Object.freeze({
    ...
    layoutHints: Object.freeze({ estimatedTextLength: String(question.blankedDisplayText).length, hasGrouping: false, avoidPageBreakInside: true, representation: "same_denominator_fraction", longTextCardPolicy: "avoidSplit" }),
  }));
  const answers = printLayout.showAnswerKeyPage ? generation.questions.map((question, index) => Object.freeze({
    ...
    layoutHints: Object.freeze({ avoidPageBreakInside: true, representation: "same_denominator_fraction_answer" }),
  })) : [];
  ...
  const document = Object.freeze({
    ...
    version: "1",
    worksheetId: `p03f15-same-denominator-${plan.questionCount}-${plan.generationSeed}`,
    worksheetKind: "batchAWorksheet",
    title: options.title ?? "三年級｜分數的加減｜同分母分數",
    subtitle: "分母不變進行分子加減，並比較同分母分數與整數1",
    ...
    publicControls: Object.freeze({ sourceId: G3B_U07_SAME_DENOMINATOR_SOURCE_ID, questionMode: "numeric", productAdmissionTask: P03F15_WORKSHEET_ADAPTER.task, globalContextRegistry: null }),
    metadata: Object.freeze({ sourceId: G3B_U07_SAME_DENOMINATOR_SOURCE_ID, knowledgePointIds: Object.freeze(knowledgePointIds), applicationExpansion: false, worksheetAdapter: P03F15_WORKSHEET_ADAPTER }),
    batchA: Object.freeze({ sourceId: G3B_U07_SAME_DENOMINATOR_SOURCE_ID, questionMode: "numeric", selectionMode: plan.selectionMode }),
  ...
  return Object.freeze({ ok: true, errors: Object.freeze([]), warnings: Object.freeze([]), worksheetDocument: document, plan, generation, validation, p03f15WorksheetAdapter: P03F15_WORKSHEET_ADAPTER });
}
