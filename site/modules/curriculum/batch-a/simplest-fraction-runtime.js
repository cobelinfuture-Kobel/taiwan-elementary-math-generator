...
function hashSeed(value) {
  let acc = 2166136261;
  for (const char of String(value ?? "p03f13")) {
...
function buildQuestion(row, index) {
  ...
  return Object.freeze({
    ...
    commonFactor: row.commonFactor,
    simplestNumerator: row.simplestNumerator,
    simplestDenominator: row.simplestDenominator,
    ...
    metadata: Object.freeze({
      ...
      patternTags: Object.freeze(["full_product_w3_slice013", definition.sourceId, definition.patternSpecId]),
...
export function canGenerateG5AU04SimplestFractionQuestions(plan = {}) {
  return plan.sourceId === G5A_U04_SOURCE_ID
    ...
    && G5A_U04_EXPAND_REDUCE_SIMPLEST_PATTERN_SPEC_IDS.every((id) => plan.patternSpecIds.includes(id));
}
export function validateG5AU04SimplestFractionQuestion(question = {}) {
  ...
  if (question.commonFactor !== factor
    || question.simplestNumerator !== question.numerator / factor
    || question.simplestDenominator !== question.denominator / factor) {
    add("p03f13_simplification_identity_invalid", "commonFactor");
  }
  if (gcd(question.simplestNumerator, question.simplestDenominator) !== 1
    || question.numerator * question.simplestDenominator !== question.simplestNumerator * question.denominator) {
    add("p03f13_simplest_fraction_invalid", "simplestNumerator");
  }
  const expectedAnswer = Number(question[question.requestedUnknownRole]);
  if (!["commonFactor", "simplestNumerator", "simplestDenominator"].includes(question.requestedUnknownRole)
    || question.answerText !== String(expectedAnswer)
    || question.finalAnswer?.value !== expectedAnswer
    || question.finalAnswer?.exact !== true) add("p03f13_answer_identity_invalid", "answerText");
...
export function generateG5AU04SimplestFractionQuestions(options = {}) {
  ...
  if (!canGenerateG5AU04SimplestFractionQuestions(plan)) {
    return { ok: false, plan, questions: [], allocation: [], errors: [{ code: "p03f13_plan_not_supported", severity: "error", path: "plan", message: "Slice013 accepts only the three admitted simplest-fraction PatternSpecs." }], warnings: [] };
  }
  if (plan.questionCount > CASES.length) {
    return { ok: false, plan, questions: [], allocation: [], errors: [{ code: "p03f13_question_count_exceeds_unique_witnesses", severity: "error", path: "questionCount", message: "Slice013 provides at most nine unique bounded witnesses." }], warnings: [] };
  }
  ...
  const allocation = G5A_U04_EXPAND_REDUCE_SIMPLEST_PATTERN_SPEC_IDS.map((patternSpecId) => ({
    patternSpecId,
