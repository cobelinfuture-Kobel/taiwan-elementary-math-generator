...
import { buildBatchABrowserWorksheetDocument as baseBuild } from "./batch-a-browser-worksheet-p03f23-extension.js";
import { buildBatchABrowserPlan, requestsP03F24 } from "./batch-a-browser-generator-p03f24.js";
import { generateBatchABrowserQuestions } from "./batch-a-browser-question-router-p03f24.js";
...
export function buildBatchABrowserWorksheetDocument(options = {}) {
  ...
  const document = Object.freeze({ schemaVersion: "worksheet-document-v1", version: "1", worksheetId: `p03f24-fraction-context-${plan.questionCount}-${plan.generationSeed}`, worksheetKind: "batchAWorksheet", title: options.title ?? "三年級下｜分數的加減｜整數與分數／情境關係", subtitle: "整數與分數加減、同一整體合併、共同單位換算與原量／差量", generatedAt: "DETERMINISTIC", configSnapshot: Object.freeze({ ...plan, printLayout: layout }), orderingMode: plan.ordering, questionCount: generation.questions.length, questionPages: Object.freeze(questionPages), answerKeyPages: Object.freeze(answerKeyPages), sections: Object.freeze([]), generatedQuestions: Object.freeze(generation.questions), questions: Object.freeze(generation.questions), questionDisplayModels: Object.freeze(models), answerKeyItems: Object.freeze(answers), printOptions: Object.freeze({ ...layout, answerKeyColumns: 2, answerKeyRowsPerPage: 4, showAnswerKey: layout.showAnswerKeyPage, answerKeyPlacement: layout.showAnswerKeyPage ? "afterQuestions" : "none" }), publicControls: Object.freeze({ sourceId: G3B_U07_SOURCE_ID, questionMode: plan.questionMode, productAdmissionTask: P03F24_WORKSHEET_ADAPTER.task, globalContextRegistry: hasApplication ? "W02_A06_EXISTING_PRODUCTION_EQUIVALENT_LINEAGE" : null }), metadata: Object.freeze({ sourceId: G3B_U07_SOURCE_ID, knowledgePointIds: Object.freeze([...new Set(generation.questions.map((question) => question.metadata.knowledgePointId))]), applicationExpansion: hasApplication, worksheetAdapter: P03F24_WORKSHEET_ADAPTER }), batchA: Object.freeze({ sourceId: G3B_U07_SOURCE_ID, questionMode: plan.questionMode, selectionMode: plan.selectionMode }), report: Object.freeze({ ok: true, errors: Object.freeze([]), warnings: Object.freeze([]), summary: Object.freeze({ questionCount: generation.questions.length, questionPageCount: questionPages.length, answerKeyPageCount: answerKeyPages.length }) }), summary: Object.freeze({ questionCount: generation.questions.length, questionPageCount: questionPages.length, answerKeyPageCount: answerKeyPages.length, numericQuestionCount, applicationQuestionCount }) });
  return Object.freeze({ ok: true, errors: Object.freeze([]), warnings: Object.freeze([]), worksheetDocument: document, plan, generation, validation, p03f24WorksheetAdapter: P03F24_WORKSHEET_ADAPTER });
}
