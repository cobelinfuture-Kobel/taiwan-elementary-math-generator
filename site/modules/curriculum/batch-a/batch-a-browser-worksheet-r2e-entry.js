...
function applyR2EContextMetadata(result,options={}){
  const document=result?.worksheetDocument;if(!result?.ok||!document||document.batchA?.sourceId!==G4B_U04_SOURCE_ID)return result;
  const questions=document.generatedQuestions??[];const contextMode=normalizeG4BU04ContextMode(options.contextMode??result.generation?.plan?.contextMode??questions.find((question)=>question.contextModeRequested)?.contextModeRequested);const contextAllocation=result.generation?.contextAllocation??summarizeG4BU04ContextAllocation(questions,contextMode);
  const publicControls={...(document.publicControls??{}),sourceId:G4B_U04_SOURCE_ID,questionMode:options.questionMode??document.batchA?.questionMode??"mixed",contextMode,layoutMode:document.layoutResolution?.layoutMode??options.layoutMode??"auto_safe",genericFallback:false,freeFormAI:false,printScope:"controlled_context_and_resolved_layout"};
  const sourceTaskIds=[...new Set([...(document.provenance?.sourceTaskIds??[]),G4B_U04_R2E_WORKSHEET_CONTEXT_INTEGRATION.task])];
  const worksheetDocument={...document,publicControls,contextAllocation:clone(contextAllocation),metadata:{...(document.metadata??{}),publicControls:clone(publicControls),contextContractVersion:G4B_U04_CONTEXT_CONTRACT_VERSION,contextMode,contextAllocation:clone(contextAllocation)},validationSummary:{...(document.validationSummary??{}),contextValidatorVersion:G4B_U04_CONTEXT_CONTRACT_VERSION},batchA:{...(document.batchA??{}),contextMode},g4bU04Summary:{...(document.g4bU04Summary??{}),contextMode,contextAllocation:clone(contextAllocation)},provenance:{...(document.provenance??{}),sourceTaskIds,contextContractVersion:G4B_U04_CONTEXT_CONTRACT_VERSION,genericContextFallbackUsed:false,freeFormAIUsed:false},configSnapshot:{...(document.configSnapshot??{}),schemaVersion:"r2e.batch_b.g4b_u04.worksheet_plan.v1",contextMode},summary:{...(document.summary??{}),contextMode,dailyLifeContextCount:contextAllocation.counts.daily_life,sdgContextCount:contextAllocation.counts.sdg,contextNotApplicableCount:contextAllocation.counts.not_applicable}};
  return {...result,worksheetDocument,contextAllocation:clone(contextAllocation)};
}
export function buildBatchABrowserWorksheetDocument(options={}){
  if(requestsP03F34(options)) return buildP03F34WorksheetDocument(options);
  if(options.sourceId===G4A_U06_P03F33_SOURCE_ID) return buildP03F33WorksheetDocument(options);
  if(options.sourceId===G6B_U01_P03F32_SOURCE_ID) return buildP03F32WorksheetDocument(options);
  if(options.sourceId===G5B_U04_P03F31_SOURCE_ID) return buildP03F31WorksheetDocument(options);
