...
import { buildBatchABrowserWorksheetDocument as baseBuild } from "./batch-a-browser-worksheet-p03f21-extension.js";
import { buildBatchABrowserPlan, requestsP03F22 } from "./batch-a-browser-generator-p03f22.js";
import { generateBatchABrowserQuestions } from "./batch-a-browser-question-router-p03f22.js";
import { validateBatchABrowserPlan, validateBatchABrowserQuestions } from "./batch-a-browser-validator-p03f22.js";
import { G5A_U04_SOURCE_ID, G5A_U04_SLICE022_KP_IDS } from "../registry/g5a-u04-rank7-fraction-selector-projection.js";

export const P03F22_WORKSHEET_ADAPTER = Object.freeze({
  task: "P03F_W3DirectProductVerticalSlice022Implementation", status: "bounded_shared_rank7_fraction_connected",
  sourceId: G5A_U04_SOURCE_ID, knowledgePointCount: 2, patternGroupCount: 2, patternSpecCount: 6,
  sharedPagination: true, sharedRenderer: true, parallelPipeline: false,
});

export function buildBatchABrowserWorksheetDocument(options = {}) {
  if (!requestsP03F22(options)) return baseBuild(options);
  ...
  const layout = Object.freeze({ paperSize: options.printLayout?.paperSize ?? "A4", columns: Math.min(options.printLayout?.columns ?? 2, 2),
    rowsPerPage: Math.min(options.printLayout?.rowsPerPage ?? 4, 4), showQuestionNumbers: options.printLayout?.showQuestionNumbers !== false,
    showAnswerKeyPage: options.includeAnswerKey !== false && options.printLayout?.showAnswerKeyPage !== false,
    longTextCardPolicy: "avoidSplit", questionMode: "numeric" });
  const models = generation.questions.map((question, index) => Object.freeze({
    ...
    answerText: question.answerText, metadataSnapshot: question.metadata,
    layoutHints: Object.freeze({ estimatedTextLength: question.blankedDisplayText.length, hasGrouping: false, avoidPageBreakInside: true,
      representation: question.operationFamilyId, longTextCardPolicy: "avoidSplit" }),
  }));
  const answers = layout.showAnswerKeyPage ? generation.questions.map((question, index) => Object.freeze({
    questionId: question.id, questionNumber: index + 1, patternId: question.patternSpecId,
    knowledgePointId: question.metadata.knowledgePointId, patternGroupId: question.metadata.patternGroupId,
    promptText: question.blankedDisplayText, answerText: question.answerText, metadataSnapshot: question.metadata,
  ...
  const document = Object.freeze({
    schemaVersion: "worksheet-document-v1", version: "1", worksheetId: `p03f22-rank7-fraction-${plan.questionCount}-${plan.generationSeed}`,
    worksheetKind: "batchAWorksheet", title: options.title ?? "五年級｜擴分約分通分｜通分與整除規則約分",
    subtitle: "通分為等值分數，並依整除特徵約成最簡分數", generatedAt: "DETERMINISTIC",
    configSnapshot: Object.freeze({ ...plan, printLayout: layout }), orderingMode: plan.ordering,
    ...
    publicControls: Object.freeze({ sourceId: G5A_U04_SOURCE_ID, questionMode: "numeric", productAdmissionTask: P03F22_WORKSHEET_ADAPTER.task, globalContextRegistry: null }),
    metadata: Object.freeze({ sourceId: G5A_U04_SOURCE_ID, knowledgePointIds: Object.freeze([...G5A_U04_SLICE022_KP_IDS]), applicationExpansion: false, worksheetAdapter: P03F22_WORKSHEET_ADAPTER }),
  ...
  return Object.freeze({ ok: true, errors: Object.freeze([]), warnings: Object.freeze([]), worksheetDocument: document, plan, generation, validation, p03f22WorksheetAdapter: P03F22_WORKSHEET_ADAPTER });
}
