...
function p03f3Plan(plan, options) {
  ...
  return {
    ...
    publicControls: { sourceId: G3B_U07_SOURCE_ID, questionMode: "numeric", productWave: "R05-W3", productAdmissionTask: "P03F_W3DirectProductVerticalSlice003Implementation", publicDropdownCutoverTask: "P03F_W3DirectProductVerticalSlice003Implementation", globalContextAuthority: "NOT_APPLICABLE" },
    publicPatternSpecInjectionUsed: false,
    genericFallbackAllowed: false,
  };
}
function p03f4Plan(plan, options) {
  const sourceUnit = getBatchASourceUnit(G3B_U09_SOURCE_ID);
  const requestedKnowledgePointIds = Array.isArray(options.selectedKnowledgePointIds) ? [...new Set(options.selectedKnowledgePointIds.filter(Boolean))] : [];
  const requestedPatternGroupIds = Array.isArray(options.selectedPatternGroupIds) ? [...new Set(options.selectedPatternGroupIds.filter(Boolean))] : [];
  return {
    ...plan,
    ...
    allocation: null,
    questionMode: "numeric",
    requestedKnowledgePointIds: requestedKnowledgePointIds.length ? requestedKnowledgePointIds : [G3B_U09_TENTH_DECIMAL_KP_ID],
    requestedPatternGroupIds: requestedPatternGroupIds.length ? requestedPatternGroupIds : [G3B_U09_TENTH_DECIMAL_PATTERN_GROUP_ID],
    publicControls: { sourceId: G3B_U09_SOURCE_ID, questionMode: "numeric", productWave: "R05-W3", productAdmissionTask: "P03F_W3DirectProductVerticalSlice004Implementation", publicDropdownCutoverTask: "P03F_W3DirectProductVerticalSlice004Implementation", globalContextAuthority: "NOT_APPLICABLE" },
    publicPatternSpecInjectionUsed: false,
    genericFallbackAllowed: false,
  };
}
...
export function buildBatchABrowserPlan(options = {}) {
  ...
  if (options.sourceId === G3B_U09_SOURCE_ID) return p03f4Plan(plan, options);
