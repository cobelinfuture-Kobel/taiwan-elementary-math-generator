...
import {generateBatchABrowserQuestions,requestsP07F19} from "./batch-a-browser-generator-p07f19.js";
import {buildBatchABrowserWorksheetDocument as baseWorksheet} from "./batch-a-browser-worksheet-p07f18-extension.js";
import {validateG6BU04P07F19Question} from "./g6b-u04-base-comparison-rate-roles-runtime-p07f19.js";
import {G6B_U04_P07F19_KP_ID as KP,G6B_U04_P07F19_SOURCE_ID as SRC} from "../registry/g6b-u04-base-comparison-rate-roles-selector-projection-p07f19.js";
...
export function buildBatchABrowserWorksheetDocument(o={}){
  if(!requestsP07F19(o))return baseWorksheet(o);
  const generation=generateBatchABrowserQuestions(o);
  if(!generation?.ok)return Object.freeze({ok:false,errors:generation?.errors??["P07F19_GENERATION_FAILED"],warnings:generation?.warnings??[],worksheetDocument:null,generation});
  const ve=generation.questions.flatMap(q=>validateG6BU04P07F19Question(q).errors);
  ...
  if(generation.questions.some(q=>q.knowledgePointId!==KP))return Object.freeze({ok:false,errors:Object.freeze(["P07F19_WORKSHEET_KP_INVALID"]),warnings:Object.freeze([]),worksheetDocument:null,generation});
  ...
  const worksheetDocument=Object.freeze({
    worksheetKind:"batch_a",worksheetId:"p07f19-"+SRC+"-"+(o.generationSeed??"public"),title:"基準量與比較量｜基準量、比較量與比率角色",
    ...
    metadata:Object.freeze({
      taskId:"P07F_W7DirectProductVerticalSlice019Implementation",sourceId:SRC,knowledgePointId:KP,questionMode:"numeric",frozenRuntimeProfile:"profile_ratio_percent",
      baseComparisonRateRoleOwnership:true,percentageRatePrerequisiteConsumed:true,priorPercentageRateOwnershipReowned:false,
      findBaseQuantityOwnership:false,findComparisonQuantityOwnership:false,findRateOwnership:false,successiveRateChangeOwnership:false,multiStageDiscountInterestOwnership:false,
      sameUnitMixedUsed:false,crossUnitMixedUsed:false,q020OrLaterTouched:false,sharedRuntimeScope:"SHARED_RUNTIME_BOUNDED"
    }),
  ...
  return Object.freeze({ok:true,errors:Object.freeze([]),warnings:Object.freeze([]),worksheetDocument,generation,p07f19Implemented:true,q018ProductMutation:false,q020OrLaterTouched:false});
}
