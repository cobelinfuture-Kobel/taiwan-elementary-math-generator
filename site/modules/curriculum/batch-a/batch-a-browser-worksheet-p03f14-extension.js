...
export const P03F14_WORKSHEET_ADAPTER = Object.freeze({
  task: "P03F_W3DirectProductVerticalSlice014Implementation",
  status: "bounded_shared_decimal_base10_worksheet_adapter_connected",
...
export function buildBatchABrowserWorksheetDocument(options = {}) {
  if (!requestsP03F14(options)) return buildBase(options);
  ...
  const models = generation.questions.map((question, index) => Object.freeze({
    ...
    layoutHints: Object.freeze({ estimatedTextLength: String(question.blankedDisplayText).length, hasGrouping: false, avoidPageBreakInside: true, representation: "decimal_base10_place_relation", longTextCardPolicy: "avoidSplit" }),
  }));
  const answers = printLayout.showAnswerKeyPage ? generation.questions.map((question, index) => Object.freeze({
    ...
    layoutHints: Object.freeze({ avoidPageBreakInside: true, representation: "decimal_base10_place_relation_answer" }),
  })) : [];
  const questionPages = paginateQuestionDisplayModels(models, printLayout);
  const answerKeyPages = printLayout.showAnswerKeyPage ? paginateAnswerKeyItems(answers, { ...printLayout, columns: 2, rowsPerPage: 5 }) : [];
  const document = Object.freeze({
    schemaVersion: "worksheet-document-v1",
    version: "1",
    worksheetId: `p03f14-decimal-base10-${plan.questionCount}-${plan.generationSeed}`,
    worksheetKind: "batchAWorksheet",
    title: options.title ?? "五年級｜整數與小數的十進位結構｜數字題",
    subtitle: "連結小數點兩側相鄰位值的10倍與十分之一關係",
    ...
    printOptions: Object.freeze({ ...printLayout, answerKeyColumns: 2, answerKeyRowsPerPage: 5, showAnswerKey: printLayout.showAnswerKeyPage, answerKeyPlacement: printLayout.showAnswerKeyPage ? "afterQuestions" : "none" }),
    publicControls: Object.freeze({ sourceId: G5B_U05_DECIMAL_BASE10_SOURCE_ID, questionMode: "numeric", productAdmissionTask: P03F14_WORKSHEET_ADAPTER.task, globalContextRegistry: null }),
    metadata: Object.freeze({ sourceId: G5B_U05_DECIMAL_BASE10_SOURCE_ID, knowledgePointIds: Object.freeze([G5B_U05_DECIMAL_BASE10_KP_ID]), applicationClassification: "APPLICATION_NOT_APPLICABLE", worksheetAdapter: P03F14_WORKSHEET_ADAPTER }),
    batchA: Object.freeze({ sourceId: G5B_U05_DECIMAL_BASE10_SOURCE_ID, questionMode: "numeric", selectionMode: plan.selectionMode }),
  ...
  return Object.freeze({ ok: true, errors: Object.freeze([]), warnings: Object.freeze([]), worksheetDocument: document, plan, generation, validation, p03f14WorksheetAdapter: P03F14_WORKSHEET_ADAPTER });
}
