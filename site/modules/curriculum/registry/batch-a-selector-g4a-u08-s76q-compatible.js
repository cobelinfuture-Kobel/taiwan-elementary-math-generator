...
export function listBatchAKnowledgePointAvailabilityBySource(sourceId) {
  if (sourceId !== SOURCE_ID) return legacy.listBatchAKnowledgePointAvailabilityBySource(sourceId);
  return {
    sourceId,
    visibleCount: CANONICAL_KP_IDS.length,
    visibleKnowledgePointIds: [...CANONICAL_KP_IDS],
    compatibilityProjection: "virtual_canonical_kp_rows_preserve_global_registry_cardinality",
  };
}
...
export function getVisiblePatternGroupsForKnowledgePoint(knowledgePointId) {
  ...
  return clone(legacy.getVisiblePatternGroupsForKnowledgePoint(knowledgePointId));
}
...
export function validateG4AU08AllCanonicalPublicSelectorProjection() {
  ...
  return Object.freeze({
    ...
    counts: Object.freeze({ knowledgePoints: 15, patternGroups: 28, patternSpecs: 33, globalRegistryRows: globalRows.length }),
  });
}
