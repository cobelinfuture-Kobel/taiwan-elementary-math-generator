export * from "./batch-a-selector-p06f20-extension.js";
import * as base from "./batch-a-selector-p06f20-extension.js";
import {G6A_U05_P07F01_KP_ID as KP,G6A_U05_P07F01_PROTECTED_FUTURE_KP_IDS as PROTECTED,G6A_U05_P07F01_SOURCE_ID as SRC,getG6AU05P07F01SelectorRow as row,listG6AU05P07F01PatternGroups as groups,resolveG6AU05P07F01PatternSpecIds as specs} from "./g6a-u05-ratio-notation-order-selector-projection-p07f01.js";
...
if(pv.includes(KP)||PROTECTED.some(id=>pv.includes(id)))throw new Error("P07F01_BASE_VISIBILITY_INVALID");
...
const source=Object.freeze({...p,sourceId:SRC,visibleCount:visible.length,hiddenPendingCount:hidden.length,notSelectableCount:not.length,visibleKnowledgePointIds:visible,hiddenPendingKnowledgePointIds:hidden,notSelectableKnowledgePointIds:not,publicSelectorStatus:"w7_slice001_g6a_u05_ratio_notation_promoted",publicDropdownCutoverTask:"P07F_W7DirectProductVerticalSlice001Implementation",q001AddedKnowledgePointIds:Object.freeze([KP]),remainingProtectedKnowledgePointIds:PROTECTED,sameSourceCandidateSetComplete:false,sameUnitMixedAllowed:false,w7FrozenQueueComplete:false});
...
export function auditP07F01PublicSelectorComposition(){
  const e=[],ba=base.auditP06F20PublicSelectorComposition?.(),s=listBatchAKnowledgePointAvailabilityBySource(SRC),ids=listVisibleBatchAKnowledgePoints().filter(x=>x.sourceId===SRC).map(x=>x.knowledgePointId);
  if(ba&&!ba.ok)e.push(...ba.errors.map(x=>"P07F01_BASE:"+x));
  if(!ids.includes(KP)||!getVisibleBatchAKnowledgePoint(KP))e.push("P07F01_TARGET_VISIBLE_MISSING");
  if(s.hiddenPendingKnowledgePointIds.includes(KP)||s.notSelectableKnowledgePointIds.includes(KP))e.push("P07F01_TARGET_STILL_HIDDEN");
  if(PROTECTED.some(id=>ids.includes(id)||!s.hiddenPendingKnowledgePointIds.includes(id)||!s.notSelectableKnowledgePointIds.includes(id)))e.push("P07F01_PROTECTED_VISIBILITY_INVALID");
  if(s.sameSourceCandidateSetComplete!==false||s.sameUnitMixedAllowed!==false||s.w7FrozenQueueComplete!==false)e.push("P07F01_SOURCE_SCOPE_INVALID");
  if(BATCH_A_SELECTOR_AVAILABILITY.sourceCount!==Number(b.sourceCount??0)+sourceAdded)e.push("P07F01_SOURCE_COUNT_INVALID");
  return Object.freeze({ok:e.length===0,errors:Object.freeze(e),counts:Object.freeze({q001VisibleDelta:1,sourceDelta:sourceAdded,sourceVisibleCount:s.visibleCount,remainingProtected:PROTECTED.length})});
}
