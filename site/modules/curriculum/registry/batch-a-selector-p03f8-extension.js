...
const rows = Object.freeze(listG3BU09DecimalComposeDecomposeSelectorRows().map((row) => Object.freeze(row)));
const baseG3BU09 = base.listBatchAKnowledgePointAvailabilityBySource(G3B_U09_SOURCE_ID);

export { G3B_U09_DECIMAL_COMPOSE_DECOMPOSE_SELECTOR_PROJECTION };
...
export const BATCH_A_SELECTOR_AVAILABILITY = Object.freeze({
  ...
  bySourceId: Object.freeze({
    ...
    [G3B_U09_SOURCE_ID]: Object.freeze({
      ...
      publicSelectorStatus: "slice008_two_w3_knowledge_points_d0_visible",
...
export function getVisibleBatchAKnowledgePoint(id) {
  return getG3BU09DecimalComposeDecomposeSelectorRow(id) ?? base.getVisibleBatchAKnowledgePoint(id);
}
export function getVisiblePatternGroupsForKnowledgePoint(id) {
  const groups = listG3BU09DecimalComposeDecomposePatternGroups(id);
...
export function resolveVisiblePatternSpecIdsForKnowledgePoint(id) {
  const ids = resolveG3BU09DecimalComposeDecomposePatternSpecIds(id);
...
export function auditP03F8PublicSelectorComposition() {
  const errors = [...auditG3BU09DecimalComposeDecomposeSelectorProjection().errors];
  ...
  if (!availability || availability.visibleCount !== 2 || availability.hiddenPendingCount !== 5) errors.push("P03F8_SELECTOR_AVAILABILITY_INVALID");
  ...
  if (sourceRows.length !== 2 || groups.length !== 2 || specs.length !== 2) errors.push("P03F8_CURRENT_SOURCE_SURFACE_INVALID");
  return Object.freeze({
    ok: errors.length === 0,
    errors: Object.freeze(errors),
    counts: Object.freeze({ addedKnowledgePoints: 1, currentSourceKnowledgePoints: 2, currentSourcePatternGroups: 2, currentSourcePatternSpecs: 2, publicSources: 23 }),
  });
}
