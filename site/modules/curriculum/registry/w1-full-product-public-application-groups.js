...
const rows = [
  ...
  ["p01e_app_pg_g5b_u05_decompose", "pg_g5b_u05a_large_number_decompose_compare", "g5b_u05_5b05a", "kp_g5b_u05a_large_number_decompose_compare", "ps_g5b_u05a_large_number_expanded_form", "REL_DECOMPOSE_DATA_TOTAL_V1"],
  ...
  ["p01e_app_pg_g5a_u03_bounded_multiple", "pg_g5a_u03a_bounded_or_nearest_multiple", "g5a_u03_5a03a", "kp_g5a_u03a_bounded_or_nearest_multiple", "ps_g5a_u03a_list_multiples_in_interval", "REL_PACKAGE_TOTAL_OPTIONS_V1"],
...
export const W1_FULL_PRODUCT_PUBLIC_APPLICATION_GROUPS = Object.freeze(rows.map(([
  ...
  representationTags: Object.freeze([
    ...
    "controlled_semantic_application",
    "full_product_w1",
  ]),
...
export function listW1FullProductPublicApplicationGroupsForKnowledgePoint(knowledgePointId) {
  return clone(byKnowledgePoint.get(knowledgePointId) ?? []);
}
export function listW1FullProductPublicApplicationGroupsForSource(sourceId) {
  return clone(bySource.get(sourceId) ?? []);
}
export function getW1FullProductPublicApplicationGroup(patternGroupId) {
  return clone(bySelectionId.get(patternGroupId) ?? null);
}
export function listSelectedW1FullProductPublicApplicationGroups(patternGroupIds = []) {
  return [...new Set(patternGroupIds)]
    .map(getW1FullProductPublicApplicationGroup)
    .filter(Boolean);
}

export function auditW1FullProductPublicApplicationGroups() {
  const errors = [];
  if (W1_FULL_PRODUCT_PUBLIC_APPLICATION_GROUPS.length !== 13) {
    errors.push("P01E_APPLICATION_GROUP_COUNT_INVALID");
  }
  if (new Set(W1_FULL_PRODUCT_PUBLIC_APPLICATION_GROUPS.map((row) => row.primaryKnowledgePointId)).size !== 13) {
    errors.push("P01E_APPLICATION_KP_IDENTITY_INVALID");
  }
  if (new Set(W1_FULL_PRODUCT_PUBLIC_APPLICATION_GROUPS.map((row) => row.patternGroupId)).size !== 13) {
    errors.push("P01E_APPLICATION_GROUP_IDENTITY_INVALID");
  }
  return Object.freeze({
    ok: errors.length === 0,
    errors: Object.freeze(errors),
    applicationKnowledgePointCount: 13,
