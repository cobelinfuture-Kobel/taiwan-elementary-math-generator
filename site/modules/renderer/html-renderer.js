...
import { renderCircleAreaDerivationDiagram } from "./circle-area-derivation-diagram-p07f11.js";
import { renderAnnulusAreaDiagram } from "./annulus-area-diagram-p07f17.js";
...
function renderGeometryDiagram(model) {
  ...
  if (model?.kind === "circle_area_derivation_diagram") return renderCircleAreaDerivationDiagram(model);
  if (model?.kind === "annulus_area_diagram") return renderAnnulusAreaDiagram(model);
  if (model?.kind === "cylinder_volume_diagram") return renderCylinderVolumeDiagram(model);
  if (model?.kind === "scale_area_change_diagram") return renderScaleAreaChangeDiagram(model);
  ...
  if (model?.kind === "prism_pyramid_elements_diagram") return renderPrismPyramidElementsDiagram(model);
  if (model?.kind === "solid_net_correspondence_diagram") return renderSolidNetCorrespondenceDiagram(model);
  if (model?.kind === "cube_cuboid_net_diagram") return renderCubeCuboidNetDiagram(model);
  throw createRendererError("geometry_diagram_invalid", `Unsupported geometry diagram kind '${model?.kind ?? "missing"}'.`);
}

function renderTableData(model) {
  if (model?.kind === "tabular_pattern_table") return renderTabularPatternTable(model);
  if (model?.kind === "one_way_statistics_table") return renderOneWayStatisticsTable(model);
...
function renderChartData(model) {
  ...
  if (model?.kind === "pie_chart_comparison_data") return renderPieChartComparisonData(model);
