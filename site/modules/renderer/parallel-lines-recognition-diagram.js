...
import {isG5AU05Q056Diagram,renderG5AU05Q056Diagram} from "./g5a-u05-polygon-triangulation-diagram-p05f56.js";
...
function validModel(model) {return Boolean(model)&&model.kind === "parallel_lines_recognition_diagram"&&ALLOWED_ORIENTATIONS.has(model.orientationDeg)&&ALLOWED_GAPS.has(model.gapPx)&&ALLOWED_X_SHIFTS.has(model.shiftX)&&ALLOWED_MODES.has(model.diagramMode);}
...
export function renderParallelLinesRecognitionDiagram(model) {
  if (isG5AU05Q056Diagram(model)) return renderG5AU05Q056Diagram(model);
  ...
  const {point}=geometry(model);const main=[-1,1].map((sign)=>lineMarkup(point,sign,-52,52,"parallel-lines-recognition-diagram__line",'stroke="currentColor" stroke-width="3" stroke-linecap="round" '));const extensions=[];
  if(model.diagramMode==="EXTENSION_GUIDES"){for(const sign of [-1,1]){extensions.push(lineMarkup(point,sign,-72,-54,"parallel-lines-recognition-diagram__extension",'stroke="currentColor" stroke-width="2" stroke-dasharray="5 4" '));extensions.push(lineMarkup(point,sign,54,72,"parallel-lines-recognition-diagram__extension",'stroke="currentColor" stroke-width="2" stroke-dasharray="5 4" '));}}
  const arrows=[];if(model.diagramMode==="DIRECTION_ARROWS"){for(const sign of [-1,1]){const p=point(sign,-6);arrows.push(`<text class="parallel-lines-recognition-diagram__direction-arrow" x="${fixed(p.x)}" y="${fixed(p.y+4)}" text-anchor="middle" font-size="18" font-weight="700" transform="rotate(${model.orientationDeg} ${fixed(p.x)} ${fixed(p.y)})">→</text>`);}}
  return [`<div class="worksheet-cell__representation worksheet-cell__representation--parallel-lines-recognition" data-representation="parallel-lines-recognition-diagram" data-diagram-mode="${model.diagramMode}">`,'<svg class="worksheet-parallel-lines-recognition-diagram" viewBox="0 0 240 140" width="100%" height="112" role="img" aria-label="兩條互相平行的直線圖示" preserveAspectRatio="xMidYMid meet">',...extensions,...main,...arrows,"</svg>","</div>"].join("");
}
